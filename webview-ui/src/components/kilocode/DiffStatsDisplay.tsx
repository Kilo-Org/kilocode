...
export interface DiffStatsDisplayProps {
	added: number
	removed: number
	/** Maximum number of boxes to display (default: 5) */
	maxBoxes?: number
 ...
 */
const DiffStatsDisplay = ({ added, removed, maxBoxes = 5, className }: DiffStatsDisplayProps) => {
	...
	return (
		<StandardTooltip content={tooltipText}>
			<div className={cn("flex items-center gap-1.5", className)}>
				{/* Lines added */}
				<span className="text-xs font-medium text-vscode-charts-green">+{added}</span>

				{/* Lines removed */}
				<span className="text-xs font-medium text-vscode-charts-red">−{removed}</span>
