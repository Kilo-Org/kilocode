...
// import { FreeModelsInfoView } from "../FreeModelsLink" // kilocode_change - component not found
import { useQuery } from "@tanstack/react-query"
// import { getKiloBaseUriFromToken } from "@roo/kilocode/token" // kilocode_change - module not found, commenting out
// import { telemetryClient } from "@/utils/TelemetryClient" // kilocode_change - not used after commenting out fetch
...
export const KiloModelInfoView = ({
	...
	const { data: modelStats } = useQuery<{ model: string; cost: Intl.StringNumericLiteral | number }[]>({
		queryKey: ["modelstats"],
		queryFn: async () => {
			// kilocode_change - commenting out due to missing getKiloBaseUriFromToken
			// try {
			// 	return (
	...
	const averageCost = Array.isArray(modelStats) ? modelStats.find((ms: any) => ms.model === modelId)?.cost : undefined
	...
	return (
		<>
			...
			{apiConfiguration.apiProvider === "kilocode" &&
				...
				{
					/* <FreeModelsInfoView modelId={modelId} origin="settings" /> kilocode_change - component not found */
				}}
				...
				{typeof averageCost !== "undefined" && (
					<StandardTooltip content={t("kilocode:settings.modelInfo.averageKiloCodeCostDescription")}>
						<div>
							...
							{formatPrice(typeof averageCost === "string" ? parseFloat(averageCost) : averageCost)} / M
							tokens
						</div>
					</StandardTooltip>
				)}
