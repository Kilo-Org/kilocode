...
export const KiloProviderRouting = ({ apiConfiguration, setApiConfigurationField, kilocodeDefaultModel }: Props) => {
	...
	const onValueChange = (value: string) => {
		...
		setApiConfigurationField(
			...
			preference?.type === "specific"
				? (preference as { type: "specific"; provider: string }).provider
				: OPENROUTER_DEFAULT_PROVIDER_NAME,
		)
	}

	const specificProvider = apiConfiguration.openRouterSpecificProvider
	const specificProviderIsSet = !!specificProvider && specificProvider !== OPENROUTER_DEFAULT_PROVIDER_NAME
	const specificProviderIsInvalid = specificProviderIsSet && !providers.find((p) => p.label === specificProvider)
			...
			<Select value={JSON.stringify(getProviderPreference(apiConfiguration))} onValueChange={onValueChange}>
				...
				<SelectContent>
					...
					{specificProviderIsInvalid && (
						<ProviderSelectItem value={{ type: "specific", provider: specificProvider }}>
							{specificProvider}
						</ProviderSelectItem>
					)}
