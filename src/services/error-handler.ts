...
export class Logger {
	 ...
	 */
	static createError(code: string, component: string, message: string, context?: any): DiffSystemError {
	 ...
	 */
	static handleError(component: string, error: unknown, context?: any): DiffSystemError | null {
		if (error instanceof Error) {
			const diffError = Logger.createError("UNKNOWN_ERROR", component, error.message, context)
			diffError.stack = error.stack
			return diffError
		} else if (typeof error === "string") {
			return Logger.createError("UNKNOWN_ERROR", component, error, context)
		} else {
			return Logger.createError("UNKNOWN_ERROR", component, "Unknown error occurred", context)
		}
	}
	 ...
	 */
	private static log(level: LogEntry["level"], component: string, message: string, data?: any, error?: Error): void {
	...
	static async retry<T>(
		...
		for (let attempt = 0; attempt <= maxRetries; attempt++) {
			try {
				...
				lastError = Logger.handleError(component, error) ?? undefined
