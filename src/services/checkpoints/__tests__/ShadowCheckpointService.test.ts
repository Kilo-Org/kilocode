...
const initWorkspaceRepo = async ({
	workspaceDir,
	userName = "CodeFlux AI",
	userEmail = "support@codefluxai.ai",
...
describe.each([
	...
	describe(`${klass.name}#renameNestedGitRepos`, () => {
		it("handles nested git repositories during initialization", async () => {
			...
			await mainGit.init()
			await mainGit.addConfig("user.name", "CodeFlux AI")
			await mainGit.addConfig("user.email", "support@codefluxai.ai")
			...
			await nestedGit.addConfig("user.name", "CodeFlux AI")
			await nestedGit.addConfig("user.email", "support@codefluxai.ai")
...
describe("ShadowCheckpointService", () => {
	...
	describe("getTaskStorage", () => {
		...
		it("returns undefined when workspace repo exists but has no task branch", async () => {
			...
			await git.init()
			await git.addConfig("user.name", "CodeFlux AI")
