...
export class CommitMessageProvider implements vscode.Disposable {
	...
	private async handleJetBrainsCommand(...args: JetbrainsGenerationRequest): Promise<CommitMessageResult> {
		console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: Raw args received:", args)
		console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: Args length:", args.length)

		// JetBrains sends args as a nested array: [[workspacePath, selectedFiles]]
		// We need to extract from the first element
		const firstArg = args[0]
		console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: First arg:", firstArg)
		...
		if (Array.isArray(firstArg) && firstArg.length === 2) {
			...
			console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: Using nested array format")
		} else {
			// Handle direct format: [workspacePath, selectedFiles] (fallback)
			;[workspacePath, selectedFiles] = args
			console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: Using direct format")
		}

		console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: Extracted workspacePath:", workspacePath)
		console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: Extracted selectedFiles:", selectedFiles)
		console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: selectedFiles length:", selectedFiles?.length)

		const request = {
			workspacePath,
			selectedFiles,
		}
		console.log("🔧 CommitMessageProvider.handleJetBrainsCommand: Final request:", JSON.stringify(request, null, 2))
