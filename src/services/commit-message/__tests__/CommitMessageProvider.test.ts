...
describe("CommitMessageProvider", () => {
	...
	describe("generateCommitMessageExternal", () => {
		it("should return success result with generated message", async () => {
			...
			const result = await commitMessageProvider.generateCommitMessageExternal({
		...
		it("should return error when no workspace path provided", async () => {
			const result = await commitMessageProvider.generateCommitMessageExternal()

			expect(result).toEqual({
				error: "Workspace path is required for external requests",
				message: "",
			})
		})
