...
import { DiffResult, DiffLine, StreamingDiffOptions, DiffOverlay, FileBuffer } from "../../types/diff-types"
...
export class DiffEngine {
	 ...
	 */
	static createUnifiedDiff(original: string, modified: string, options: { contextLines?: number } = {}): DiffResult {
		try {
			const contextLines = options.contextLines || 3
			const patches = diff.createPatch("file", original, modified, undefined, undefined, {
	 ...
	 */
	static createFromUnifiedDiff(unifiedDiff: string): DiffResult {
		try {
	 ...
	 */
	private static processLargeFileInChunks(
		original: string,
		modified: string,
		chunkSize: number,
			...
			setTimeout(async () => {
				try {
					...
					for (let i = 0; i < totalChunks; i++) {
						const chunkResult = this.createUnifiedDiff(originalChunks[i] || "", modifiedChunks[i] || "")
	 ...
	 */
	static createOverlaysFromResult(diffResult: DiffResult, shadowBufferId: string): DiffOverlay[] {
