...
export class FileOpenerService {
  ...
  async openMultipleFiles(filePaths: string[]): Promise<OpenMultipleFilesResponse> {
    try {
      ...
      Logger.info('FileOpenerService.openMultipleFiles', `Opened ${results.filter(r => r.success).length}/${filePaths.length} files successfully`);
  ...
  getActiveTab(filePath: string): vscode.TextEditor | undefined {
    const fileBufferId = this.generateFileBufferId(filePath);
    const tab = this.activeTabs.get(fileBufferId);
    return tab?.document ? vscode.window.visibleTextEditors.find(editor => editor.document === tab.document) : undefined;
  }
   ...
   */
  private onVisibleEditorsChanged(event: vscode.TextEditorChangeEvent[]): void {
