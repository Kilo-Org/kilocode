...
export class OpusFindingsParser {
	...
	public parse(opusResponse: string): OpusTherapeuticFindings {
		...
		const insightMatch = opusResponse.match(/Insight therapeutique.*?:([\s\S]*?)(?:Score|Fixes|Commentaires)/i)
		if (insightMatch) {
		...
		const commentMatches = opusResponse.match(/\/\/\s*OPUS:\s*([A-Z_]+):\s*(.+?)\s*->\s*pattern:\s*([A-Z_]+)/g)
		if (commentMatches) {
...
`
	...
	private enrichMentalProcess(
		...
		return {
			...
			actionPlan,
			therapyPhase,
			confidence: opusFindings.confidence > 0.8 ? "FORT" : opusFindings.confidence > 0.5 ? "MOYEN" : "FAIBLE",
			timestamp: Date.now(),
		}
	}
