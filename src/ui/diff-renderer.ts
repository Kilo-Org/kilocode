...
export class DiffRenderer {
  ...
  setColorScheme(scheme: 'vscode' | 'custom' | 'high-contrast'): void {
    ...
    this.render(); // Re-render with new color scheme
  }
  ...
  render(overlays: DiffOverlay[]): void {
    try {
      ...
      this.activeEditor.setDecorations(decorations);
  ...
  getOverlayAt(position: vscode.Position): DiffOverlay | undefined {
    ...
    return this.overlays.find(overlay => 
      line >= overlay.startLine && line <= overlay.endLine
    );
  }
  ...
  private createDecoration(overlay: DiffOverlay): vscode.DecorationOptions | null {
    try {
      ...
      // Adjust styling based on acceptance state
      if (overlay.isAccepted) {
        renderOptions.backgroundColor = this.adjustColorBrightness(colors.backgroundColor, 0.3);
        renderOptions.border = `1px solid ${this.adjustColorBrightness(colors.borderColor, 0.5)}`;
        renderOptions.opacity = 0.8;
      } else if (overlay.isRejected) {
        renderOptions.backgroundColor = this.adjustColorBrightness(colors.backgroundColor, -0.2);
        renderOptions.border = `1px solid ${this.adjustColorBrightness(colors.borderColor, -0.3)}`;
        renderOptions.opacity = 0.4;
      }
