...
export class InteractionLayer {
  ...
  constructor() {
    ...
    DiffEventManager.onOverlayStateChanged(this.onOverlayStateChanged.bind(this));
  }
  ...
  navigateToNextOverlay(): boolean {
    try {
      ...
      const nextOverlay = overlays.find(overlay => 
        overlay.startLine > currentPosition.line
      );
  ...
  navigateToPreviousOverlay(): boolean {
    try {
      ...
      const previousOverlay = [...overlays].reverse().find(overlay => 
        overlay.endLine < currentPosition.line
      );
  ...
  private getOverlayAtPosition(position: vscode.Position): DiffOverlay | undefined {
    return this.overlays.find(overlay => 
      position.line >= overlay.startLine && position.line <= overlay.endLine
    );
  }
