...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		// kilocode_change start - ghostServiceSettings (storage key preserved)
		case "ghostServiceSettings":
			if (!message.values) {
				return
			}
			// Validate autocompleteServiceSettings structure (storage key preserved as ghostServiceSettings)
			const autocompleteServiceSettings = autocompleteServiceSettingsSchema.parse(message.values)
			await updateGlobalState("ghostServiceSettings", autocompleteServiceSettings)
			await provider.postStateToWebview()
