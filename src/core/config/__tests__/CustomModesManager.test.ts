...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = `${path.sep}mock${path.sep}workspace${path.sep}.codefluxaimodes`
	...
	describe("getCustomModes", () => {
		it("should merge modes with .codefluxaimodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .codefluxaimodes since it takes precedence
		...
		it("should handle missing .codefluxaimodes file", async () => {
		...
		it("should handle invalid JSON in .codefluxaimodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .codefluxaimodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .codefluxaimodes precedence", async () => {
			...
			// Should update global state with merged modes where .codefluxaimodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .codefluxaimodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .codefluxaimodes file when adding project-specific mode", async () => {
			...
			// Mock .codefluxaimodes to not exist initially
			...
			// Verify .codefluxaimodes was created with the project mode
			...
			// Verify the content written to .codefluxaimodes
