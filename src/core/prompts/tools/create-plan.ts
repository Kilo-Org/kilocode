// kilocode_change start: Add create_plan tool description function
import { ToolArgs } from "./types"

export function getCreatePlanDescription(args: ToolArgs): string {
	return `## create_plan
Description: Create an ephemeral planning document for brainstorming, strategy, and structured thinking. The document appears as an editor tab and is fully accessible during this session, but will be automatically discarded when the editor session ends (not saved to disk).
...
Parameters:
- title: (required) The title/name of the plan document. Will be used as the filename (automatically adds .plan.md extension if not present)
