...
- Before using the execute_command tool, you must first think about the SYSTEM INFORMATION context provided to understand the user's environment and tailor your commands to ensure they are compatible with their system. Use the appropriate command chaining operator for the user's operating system: use ';' for Windows (especially older PowerShell versions) and '&&' for Unix/Linux/macOS. You must also consider if the command you need to run should be executed in a specific directory outside of the current working directory '${cwd.toPosix()}', and if so prepend with \`cd\`'ing into that directory, then use the appropriate chaining operator for the OS to execute the command (as one command since you are stuck operating from '${cwd.toPosix()}'). For example, if you needed to run \`npm install\` in a project outside of '${cwd.toPosix()}', you would need to prepend with \`cd\` i.e. pseudocode for this would be \`cd (path to project) && (command, in this case npm install)\` on Unix or \`cd (path to project) ; (command)\` on Windows.
