...
export async function* runClaudeCode(options: ClaudeCodeOptions): AsyncGenerator<ClaudeCodeMessage | string> {
	...
	if (os.platform() === "win32" || isSystemPromptTooLong) {
		...
		// Use exclusive flag (wx) to prevent TOCTOU race conditions and set restrictive permissions
...
Rules and workflows contribute to a longer system prompt, consider disabling some of them temporarily to reduce the length.`,
			...
			if (err.message.includes("ENAMETOOLONG")) {
				throw new Error(
					`Executing Claude Code failed due to a long system prompt. Windows has a limit of 8191 characters.
Please ensure the Claude Code CLI is up to date.`,
					{ cause: err },
				)
			}
...
function runProcess(
	{ systemPrompt, messages, path: claudePath, modelId, thinkingBudgetTokens, shouldUseFile }: ClaudeCodeOptions,
	cwd: string,
) {
	...
	const args = [
		...
		"stream-json",
		"--disallowedTools",
		claudeCodeDisallowedTools,
		// Kilocode will handle recursive calls
		"--max-turns",
		"1",
		"--model",
		modelId,
...
function attemptParseChunk(data: string): ClaudeCodeMessage | null {
	try {
		...
		console.error("Error parsing chunk:", data.length)
		return null
	}
}
