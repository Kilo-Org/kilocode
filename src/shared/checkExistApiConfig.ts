...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	// Special case for human-relay, fake-ai, claude-code, openai-codex, qwen-code, and roo providers which don't need any configuration.
	if (
		config.apiProvider &&
		["human-relay", "fake-ai", "claude-code", "openai-codex", "qwen-code", "roo", "gemini-cli"].includes(
			config.apiProvider,
		) // kilocode_change: add gemini-cli
	) {
		return true
	}
