# Releasing Devil Code

Devil Code uses a fully automated CI pipeline triggered via GitHub Actions `workflow_dispatch`. A single workflow handles version bumping, building all artifacts, publishing to every distribution channel, and updating package registries.

## How to Trigger a Release

1. Go to the [`publish` workflow](https://github.com/Devil-Org/devilcode/actions/workflows/publish.yml) in GitHub Actions.
...
- Runs `packages/opencode/script/build.ts` to compile the Devil CLI binary.
...
- Uploads the `dist/` directory as a workflow artifact (`devil-cli`) for subsequent jobs.

### 3. Build VS Code Extension (`build-vscode`)

- Downloads the CLI artifacts from the previous job.
- Runs `packages/devil-vscode/script/build.ts` to build VSIX packages for all target platforms:
...
- Uploads the VSIX files as a workflow artifact (`devil-vscode`).
...
- Publishes platform-specific binary packages to **npm** (e.g. `@devilcode/cli-linux-x64`, `@devilcode/cli-darwin-arm64`, etc.).
- Publishes the main `@devilcode/cli` package to **npm** with optional dependencies on the binary packages.
- Builds and pushes a multi-arch **Docker image** (`ghcr.io/devilcode/devil`) to GitHub Container Registry (linux/amd64 + linux/arm64).

#### SDK (`@devilcode/sdk`)

- Builds and publishes the TypeScript SDK to **npm**.

#### Plugin (`@devilcode/plugin`)
...
- **AUR (Arch Linux)**: Clones `devil-bin` from the AUR, updates the `PKGBUILD` with new version and SHA256 checksums, and pushes.
- **Homebrew**: Clones `devilcode/homebrew-tap`, updates the `devil.rb` formula with new version, download URLs, and SHA256 checksums, and pushes.

## Prerequisites and Permissions

### Repository Access

- The workflow only runs in the `Devil-Org/devilcode` repository (guarded by `if: github.repository == 'Devil-Org/devilcode'`).
...
| Secret                        | Purpose                                                           |
| ----------------------------- | ----------------------------------------------------------------- |
| `DEVIL_API_KEY`               | Devil API key used during version computation                     |
| `DEVIL_ORG_ID`                | Devil organization ID                                             |
| `DEVIL_MAINTAINER_APP_ID`     | GitHub App ID for the devil-maintainer bot (used for git commits) |
| `DEVIL_MAINTAINER_APP_SECRET` | GitHub App secret for the devil-maintainer bot                    |
