...
This guide provides detailed information on how to configure each provider in Coppy Code CLI. Each provider has specific configuration requirements and optional settings that can be customized to suit your needs.
...
Coppy Code CLI supports multiple AI providers, each with their own configuration requirements. This document details the configuration fields for each provider, including required and optional parameters.
...
The official Coppy Code provider for accessing Coppy Code's managed AI services.

**Description**: Access Coppy Code's managed AI infrastructure with support for multiple models and organizations.

**Required Fields**:

- `kilocodeToken` (password): Your Coppy Code authentication token
...
- [Coppy Code Documentation](https://docs.kilocode.com/)
