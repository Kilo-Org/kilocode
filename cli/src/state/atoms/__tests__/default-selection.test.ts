...
describe("default selection behavior", () => {
	describe("slash command suggestions", () => {
		...
		it("should NOT change selectedIndex when command suggestions are cleared", () => {
			const store = createStore()
			...
			// Clear suggestions - should NOT reset selectedIndex
			// This allows other suggestion types to maintain their selection
			store.set(setSuggestionsAtom, [])

			// selectedIndex should remain unchanged
			expect(store.get(selectedIndexAtom)).toBe(5)
		})
	...
	describe("argument suggestions", () => {
		...
		it("should NOT change selectedIndex when argument suggestions are cleared", () => {
			const store = createStore()
	...
	describe("file mention suggestions", () => {
		...
		it("should NOT change selectedIndex when file mention suggestions are cleared", () => {
			const store = createStore()
