	...
	try {
		...
		switch (message.type) {
			case "state":
				...
				// Filter out ask:command_output messages - say:command_output has the actual output
				if (message.state?.chatMessages) {
					const filteredMessages = message.state.chatMessages.filter(
						(msg) => !(msg.type === "ask" && msg.ask === "command_output"),
					)
			...
			case "messageUpdated": {
				...
				if (chatMessage) {
					...
					if (chatMessage.type === "ask" && chatMessage.ask === "command_output") {
						logs.debug(
							`[messageUpdated] Filtering out command_output ask, ts: ${chatMessage.ts}`,
							"effects",
						)
						break
					}
			...
			case "commandExecutionStatus": {
				...
				// Store output updates for tracking purposes
				try {
