...
class CommitMessageService {
    ...
    suspend fun generateCommitMessage(
        ...
        return try {
            logger.info("Starting commit message generation for workspace: $workspacePath")
            ...
            // Execute the command via RPC with timeout
            ...
            logger.warn("Commit message generation timed out", e)
    ...
    private suspend fun executeRPCCommand(
        ...
        // Prepare command parameters - only workspacePath needed,
        // VSCode extension handles staged/unstaged logic automatically
        val params = mapOf(
            "workspacePath" to workspacePath
        )

        // Execute the command via RPC using the correct interface
