...
Four source-backed request deltas live in the existing public HTTP request
...
The fourth delta closes the v1 request-policy gap: when the effective
`hide_prompt_training_models` setting is an explicit `true`, the hook merges
`provider.data_collection: "deny"` into every serialized Gateway request body
on all three endpoints — the same wire field v1's `createKilo` wrapper applied
through `transformRequestBody` (v1 `ecccd1f` `kilo-gateway/src/responses.ts`),
which v1 derived in `patchKiloProviderPrivacy` and the `kilo` custom loader.
The merge keeps an existing record `provider` (routing fields preserved),
replaces a missing or non-record one with exactly the marker, leaves an
already-applied body byte-identical, and never rewrites a body that does not
decode as a JSON record — an unexpressible policy is never silently promised.
The host bridges the effective value through
`GatewayOptions.dataCollectionPolicy` (`kilo-cli/src/request-policy.ts`),
resolved per location by the real settings store fold and the picker's own
project-over-profile resolution, so false, unset, or invalid values request
nothing; a stored `null` is invalid (not absent) and an invalid scope falls
back to a valid value in the other scope without inventing a reset, and a
document the host ignores natively (for example a missing `{file:...}`
reference) still supplies its raw Kilo-only value, exactly as the settings
dialog reports it. The plugin caches the resolved policy per location and re-reads it on
startup, every account refresh (credential events, `organization.set`), and
`config.updated`; a failed read keeps the last known value, and the in-memory
cell means no per-request settings I/O or external metadata fetch. This
freshness contract differs from the picker's on purpose: the picker reads the
same setting live per dialog open, while the request policy follows the host's
loaded-config lifecycle — the preview's config file watcher is off and the
settings dialog already reports `restartRequired`, so an edit reaches the wire
no earlier than the next account/config refresh and no later than restart.
Direct plugin coverage proves all three dialects, BYOK/reasoning preservation,
the allow-override, refresh, and keep-last-known. The launched-host protocol
fixture proves the marker's absence while the setting is unset across all four
source-selected protocols, then opts the profile in and proves the
account-switch refresh carries the marker to the wire; the routed-model
fixture asserts the deny marker on every wire body — title, both native Auto
routes, and the ordinary model — alongside the preserved `reasoning`/
`reasoning_effort` variant payloads.
