...
## Goal 70 renderer acceptance — 2026-09-07

The real indexing engine now has an automated renderer scenario covering In
Progress with actual per-file counts, then Complete after releasing a held
loopback embedding batch. The disabled/no-engine scenario and resize assertion
remain. Parent run: 2 scenarios pass; combined with usage: 3 pass / 6 wrapper
assertions. No component copy or invented producer state is used.
