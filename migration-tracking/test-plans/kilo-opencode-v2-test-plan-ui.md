...
| 10 | PR sidebar | UI-081 – UI-082 | 2 | NOT RUN |
| 11 | Running-shell sidebar | UI-083 | 1 | NOT RUN |
| **Total** | | **UI-001 – UI-083** | **83 Cases** | **NOT RUN** |
...
## 10. Pull-request sidebar

### UI-081: PR lookup, exact-head fallbacks and malformed output

* **Status:** NOT RUN; implementation under review.
* **Priority:** High
* **Prerequisites:** Shared disposable setup; fixture-only `gh` executable on the child PATH, isolated Git repository and no real GitHub credentials. Use `test/sidebar-pr-ui-fixture.tsx` as the fixture reference.
* **Steps:** Exercise tracking-reference lookup, branch lookup, local SHA search, fork-parent branch search and final fork SHA search. Supply a nonmatching SHA before a matching result, malformed output, and no installed `gh`. Render a long PR title at narrow and wide sizes.
* **Expected:** Display only the selected fixture PR number/title. SHA-based fallbacks require exact head equality. Malformed output does not crash or produce an unhandled rejection; unavailable results invent no PR. Record each fallback's actual invocation and result rather than inferring coverage from the initial view lookup.
* **Cleanup:** Terminate only case-owned processes and follow the shared disposable-root cleanup.
* **Evidence / Limitations:** `packages/kilo-cli/src/tui-plugin/sidebar-pr.tsx`, `packages/kilo-cli/test/sidebar-pr.test.tsx`, `packages/kilo-cli/test/sidebar-pr-ui.test.tsx`. No live account or network requests are authorized by this case.

### UI-082: PR sidebar supersede, teardown and branch-refresh limitation

* **Status:** NOT RUN; implementation under review.
* **Priority:** High
* **Prerequisites:** UI-081 fixture setup with two session locations and a controllably delayed fixture `gh` process.
* **Steps:** Switch viewed sessions during lookup, hide/show the sidebar, resize without changing location, and exit during a delayed lookup. Record started child PIDs and observe their exit. Separately change the Git branch within one location and record whether a native VCS event reaches the UI.
* **Expected:** No late result from the previous location appears. Teardown terminates and waits for case-owned lookup children; no shell/model session is created. Width-only changes do not trigger duplicate lookup. Missing same-location watcher events are recorded as an open producer limitation, not claimed fixed by the PR component; explicit remount/sync is recorded separately.
* **Cleanup:** Verify all case-owned PIDs have exited, then follow shared cleanup.
* **Evidence / Limitations:** Same component/fixture as UI-081; `kilocode/baseline/sidebar-pr-v2-parity.md`. Watcher behavior must be measured per host and repository layout, including worktrees.

### UI-083: Running-shell sidebar lifecycle and session isolation

* **Status:** NOT RUN.
* **Priority:** High
* **Prerequisites:** Shared isolated setup; harmless long-running shell command, two sessions, no account/model access required. Use `test/sidebar-processes-ui-fixture.tsx` as the executable reference.
* **Steps:** Start a session shell; observe its command and real PID. Switch sessions and return while it runs. Let it end while another session is focused, then return. Resize narrow/wide, repeat tab switches, and exit. Separately record behavior after restarting the client/server; do not infer that outcome from same-client tab tests.
* **Expected:** Only durable shell messages in the viewed session joined to live running registry entries render. Ended/missing entries disappear after reconciliation. No invented ports, readiness, or description fields. Repeated mounts do not retain event subscriptions. A tool-created shell without a durable shell-message link is a separate coverage question, not assumed visible.
* **Cleanup:** Stop and await only case-owned processes; follow shared cleanup.
* **Evidence / Limitations:** `packages/kilo-cli/src/tui-plugin/sidebar-processes.tsx`, `test/sidebar-processes.test.tsx`, `test/sidebar-processes-ui.test.tsx`. Automated `client` and `server` scenarios now exercise actual reopen/restart on an isolated host: live PID preserved on client reopen; stale durable running history hidden after server restart and resync. Manual status remains NOT RUN; this is not v1 background-process supervision parity.

## 12. Quick-Smoke Subset (~12 Essential IDs)
