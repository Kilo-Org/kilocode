...
- Read `migration-tracking/technical-notes/v2-fork-conventions.md` before branching, merging upstream, or adding Kilo code. It overrides the branch-name rule below and defines Kilo-owned paths, shared-hook limits, and `kilocode_change` markers.
- Pristine upstream v2 is already red at the pinned baseline SHA. Check `migration-tracking/technical-notes/baseline/pinned-v2-baseline.md` before attributing a failing check to your change.
