...
import { $ } from "bun"
import { createDevil } from "@devilcode/sdk/v2"
...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/Devil-Org/devilcode/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/Devil-Org/devilcode/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
...
async function summarizeCommit(opencode: Awaited<ReturnType<typeof createDevil>>, message: string): Promise<string> {
    ...
    .prompt(
      {
        ...
        model: { providerID: "kilo", modelID: "anthropic/claude-sonnet-4.5" }, // devilcode_change
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], opencode: Awaited<ReturnType<typeof createDevil>>) {
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/Devil-Org/devilcode/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const opencode = await createDevil({ port: 0 })
  const notes: string[] = []
