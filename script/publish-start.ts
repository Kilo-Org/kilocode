...
const skipNotes = process.env["DEVIL_SKIP_NOTES"] === "1" // devilcode_change
if (skipNotes) console.log("changelog skipped: DEVIL_SKIP_NOTES=1") // devilcode_change
...
if (!Script.preview) {
  ...
  // devilcode_change start - skip draft flag when DEVIL_SKIP_NOTES=1 (used by publish-stable.yml which doesn't have a publish-complete step)
