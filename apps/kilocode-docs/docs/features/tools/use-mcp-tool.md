...
The `use_mcp_tool` tool enables interaction with external tools provided by connected Model Context Protocol (MCP) servers. It extends Coppy Code's capabilities with domain-specific functionality through a standardized protocol.
...
This tool allows Coppy Code to access specialized functionality provided by external MCP servers. Each MCP server can offer multiple tools with unique capabilities, extending Coppy Code beyond its built-in functionality. The system validates arguments against schemas, manages server connections, and processes responses of various content types (text, image, resource).
...
- **Global Configuration**: Managed through the Coppy Code extension settings in VS Code. These apply across all projects unless overridden.
...
3. **Response Processing**:
   ...
   - Results are formatted for display in the Coppy Code interface
