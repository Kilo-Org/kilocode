...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows Coppy Code to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When Coppy Code needs additional context from external systems
- When Coppy Code needs to access domain-specific data from specialized MCP servers
- When Coppy Code needs to retrieve reference documentation hosted by MCP servers
- When Coppy Code needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
   ...
   - Returns the processed resource data to Coppy Code for use in the current task
...
- When helping with API development, Coppy Code retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, Coppy Code accesses current data samples from connected MCP servers.
- When working in specialized domains, Coppy Code retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, Coppy Code references compliance requirements from documentation resources.
