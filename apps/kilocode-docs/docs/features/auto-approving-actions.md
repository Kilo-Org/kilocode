...
> ⚠️ **SECURITY WARNING:** Auto-approve settings bypass confirmation prompts, giving Coppy Code direct access to your system. This can result in **data loss, file corruption, or worse**. Command line access is particularly dangerous, as it can potentially execute harmful operations that could damage your system or compromise security. Only enable auto-approval for actions you fully trust.
...
2. Select which actions Coppy Code can perform without asking permission
...
| ------------------------------ | ------------------------------------------------ | ----------- |
| **Read files and directories** | Lets Coppy Code access files without asking       | Medium      |
| **Edit files**                 | Lets Coppy Code modify files without asking       | **High**    |
...
| **Use MCP servers**            | Lets Coppy Code use configured MCP services       | Medium-High |
| **Switch modes**               | Changes between Coppy Code modes automatically    | Low         |
...
> **Allow Coppy Code to automatically perform operations without requiring approval. Enable these settings only if you fully trust the AI and understand the associated security risks.**
...
**Description:** "When enabled, Coppy Code will automatically view directory contents and read files without requiring you to click the Approve button."

**Risk level:** Medium

While this setting only allows reading files (not modifying them), it could potentially expose sensitive data. Still recommended as a starting point for most users, but be mindful of what files Coppy Code can access.
:::
...
This setting allows Coppy Code to modify your files without confirmation. The delay timer is crucial:
...
Allows Coppy Code to control a headless browser without confirmation. This can include:
...
This setting automatically retries API calls when they fail. The delay controls how long Coppy Code waits before trying again:
...
Allows Coppy Code to change between different modes (Code, Architect, etc.) without asking for permission. This primarily affects the AI's behavior rather than system access.
:::
...
Enables Coppy Code to create and complete subtasks automatically. This relates to workflow organization rather than system access.
:::
...
**Description:** Automatically selects the first AI-suggested answer for a follow-up question after a configurable timeout. This speeds up your workflow by letting Coppy Code proceed without manual intervention.
...
- Overnight runs where you want Coppy Code to continue working
...
This setting allows Coppy Code to automatically update task progress and todo lists during work sessions. This includes:
...
This is particularly useful when combined with the Subtasks permission, as it allows Coppy Code to maintain a complete picture of project progress without constant approval requests.
:::
...
**"You Only Live Once"** mode enables _all_ auto-approve permissions at once using the master toggle. This gives Coppy Code complete autonomy to read files, write code, execute commands, and perform any operation without asking for permission.
...
This is the fastest way to work with Coppy Code, but also the riskiest. Use it only when you fully trust the AI and are prepared for the consequences.
:::
