...
This guide covers common issues when using Coppy Code in JetBrains IDEs (IntelliJ IDEA, Android Studio, WebStorm, PyCharm, etc.).

## Known Missing Features

The following features, available in the VS Code version of Coppy Code, are not currently implemented in the JetBrains version:
...
The JetBrains Kilo Extension requires Node.js to be installed on your system. Node.js is used to run the extension's backend services and handle communication between the IDE and Coppy Code's AI features.
...
JCEF (Java Chromium Embedded Framework) is required for Coppy Code's web-based interface to display properly in JetBrains IDEs. Most JetBrains IDEs include JCEF support by default, but some configurations may need manual activation.
...
_For general Coppy Code support and documentation, visit [kilocode.ai/docs](https://kilocode.ai/docs)_
