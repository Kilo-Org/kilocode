...
Prompt engineering is the art of crafting effective instructions for AI models like Coppy Code.  Well-written prompts lead to better results, fewer errors, and a more efficient workflow.

## General Principles

*   **Be Clear and Specific:** Clearly state what you want Coppy Code to do. Avoid ambiguity.
    *   **Bad:** Fix the code.
...
It's often helpful to guide Coppy Code through a "think-then-do" process:

1.  **Analyze:** Ask Coppy Code to analyze the current code, identify problems, or plan the approach.
2.  **Plan:**  Have Coppy Code outline the steps it will take to complete the task.
3.  **Execute:**  Instruct Coppy Code to implement the plan, one step at a time.
4.  **Review:**  Carefully review the results of each step before proceeding.

## Using Custom Instructions

You can provide custom instructions to further tailor Coppy Code's behavior. There are two types of custom instructions:
...
*   Adjust Coppy Code's tone or personality.

See the [Custom Instructions](/advanced-usage/custom-instructions) section for more details.

## Handling Ambiguity

If your request is ambiguous or lacks sufficient detail, Coppy Code might:
...
If Coppy Code doesn't produce the desired results, you can provide feedback by:

*   **Rejecting Actions:** Click the "Reject" button when Coppy Code proposes an action you don't want.
*   **Providing Explanations:** When rejecting, explain *why* you're rejecting the action.  This helps Coppy Code learn from its mistakes.
...
By following these tips, you can write effective prompts that get the most out of Coppy Code's capabilities.
