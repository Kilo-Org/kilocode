...
# Using v0 With Coppy Code

Coppy Code supports v0, Vercel's AI model provider that offers an OpenAI-compatible API. This allows you to use v0's models with Coppy Code through the familiar OpenAI API interface.

## Prerequisites

To use v0 with Coppy Code, you'll need:
...
Setting up v0 in Coppy Code is straightforward:

1. In Coppy Code settings (click the <Codicon name="gear" /> icon):
...
<img src="/docs/img/providers/v0-setup.png" alt="v0 configuration in Coppy Code settings" width="600" />
