...
You'll find these settings in the Axon Code settings panel (click the <Codicon name="gear" /> icon):
...
Axon Code supports full endpoint URLs in the Base URL field, providing greater flexibility for provider configuration:
...
While this provider type allows connecting to various endpoints, if you are connecting directly to the official OpenAI API (or an endpoint mirroring it exactly), Axon Code recognizes the following model IDs based on the `openAiNativeModels` definition in its source code:
...
By using an OpenAI-compatible provider, you can leverage the flexibility of Axon Code with a wider range of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
