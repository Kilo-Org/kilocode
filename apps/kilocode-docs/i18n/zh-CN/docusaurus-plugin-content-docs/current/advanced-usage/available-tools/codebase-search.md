...
- 当 Coppy Code 需要在项目中查找与特定功能相关的代码时
- 当查找实现模式或相似代码结构时
...
- 当 Coppy Code 实现新功能时，搜索 "身份验证中间件" 以在编写新代码之前了解现有模式。
- 当调试问题时，Coppy Code 搜索 "API 调用中的错误处理" 以查找代码库中相关的错误模式。
- 当重构代码时，Coppy Code 搜索 "数据库事务模式" 以确保所有数据库操作的一致性。
- 当加入新代码库时，Coppy Code 搜索 "配置加载" 以了解应用程序如何引导。
