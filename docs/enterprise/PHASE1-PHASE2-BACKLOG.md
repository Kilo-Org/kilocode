...
对齐 [二次开发计划.md](./二次开发计划.md) v3.0 与 [软件外包开发合同](./软件外包开发合同_企业私有化AI编程工具.md) §2.4 第二批次；Phase 2（M3）目标：**License 完整 + RBAC 三员 + 后台 8 模块 + 国产模型（含 Minimax）+ 灰度 Fallback**。

## P0 — L3 控制面 + L4 生产入口

| 架构模块 | 说明 |
|---|---|
| L3 License & 订阅引擎 | PG+Redis；在线/离线；只读降级；虚线 → L2 API 抽象 |
| L3 RBAC | 多租户、三员；Auth 实线接入；虚线 → L2 API 抽象 |
| L3 模型配置管理 | Endpoint/Fallback/配额；虚线 → L2 配置翻译 |
| L3 管理后台 | Ant Design Pro |
...
| L2 国产模型适配层 | Qwen/DeepSeek/GLM → LLM 集群 |
| RSA/SM2 离线验签 | 关闭 Phase 1 占位签名 |
| L5 VS Code | 默认经 L4，关闭 POC 直连开关 |
...
| ID | 项 |
|---|---|
| P1-W2-02 | 独立 `enterprise-bridge` 仓库 |
| P1-W6-04 | 本安全评审纪要签字 |
