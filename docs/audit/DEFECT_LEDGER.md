...
| D-030 | KiloProvider/Extension (all) | High | Tab panel KiloProvider instances never receive V4 services — all V4 subsystem tabs dead in "Open in Tab" and restored tabs | `extension.ts`: `openKiloInNewTab()` (line 523) and tab panel serializer (line 222) both create KiloProvider without calling `setV4Services()`. All V4 service refs remain null; message handlers silently no-op. | Fixed | Added `setV4Services()` calls to both `openKiloInNewTab()` and deserializer. Passed services bundle via function parameter. | `extension.ts:223-234,545-556` | Builder | Pending Confirmer |
| D-031 | Governance (H/68) | High | Governance subsystem registration never called — adversarial audit guaranteed to fail | `GovernanceService.registerSubsystem()` exists but was never called anywhere. The `registeredSubsystems` Map remained permanently empty. Additionally, expected subsystem names in the audit didn't match actual V4 services. | Fixed | Added 10 `registerSubsystem()` calls in `extension.ts` after service init. Updated `expectedSubsystems` list in `GovernanceService` to match actual V4 services. | `extension.ts:90-99`, `GovernanceService.ts:1097-1106` | Builder | Pending Confirmer |
| D-032 | SSH/VPS (A/17,B/27) | Medium | VPS and SSH services not linked — VPSService spawns its own SSH child processes instead of using SSHService | VPSService defines its own `SSHService` interface (line 96-99) that the real SSHService doesn't satisfy. `new VPSService(context)` never receives `sshService` as second argument. VPS uses `TerminalSSHRunner` with weaker `StrictHostKeyChecking=no`. | Open | Wire `sshService` into VPSService constructor; unify SSH interface | -- | -- | -- |
| D-033 | ZeroClaw/Routing (C/35,D/45) | Medium | ZeroClaw does not route through RoutingService — executes tasks via direct terminal, not AI providers | ZeroClawService `executeTask()` opens a VS Code terminal with the task description as a shell argument. No provider selection, no routing, no model invocation. | Open | Architecture decision: ZeroClaw is currently a terminal task runner, not an AI task delegator | -- | -- | -- |
