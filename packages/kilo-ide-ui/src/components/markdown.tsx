...
// Mermaid rendering and morphdom guards for highlighted blocks
...
type RenderedBlock =
  ...
  | {
      ...
      mode: "code"
      raw: string
      src: string // Mermaid consumes delimiter-free source while raw preserves stream identity
      hash: string
      language: string
      complete: boolean
      generation: number
...
export function Markdown(
    ...
    async (src) => {
      ...
      return Promise.all(
        src.projection.blocks.map(async (block, index) => {
          ...
          if (block.mode === "code") {
            // mermaid blocks are rendered as diagrams by
  ...
  // generation counter prevents stale deferredHighlight
  ...
  createEffect(() => {
    ...
    // progressive Shiki highlighting for non-streaming
  ...
  // progressive Shiki highlighting (issue #6221, PR #7102).
  ...
  onCleanup(() => {
    // cancel any in-flight deferredHighlight pass so its
...
function updateCodeBlock(
  ...
  // mermaid blocks render as a source <pre> for
  ...
  if (block.language === "mermaid") {
    ...
    codeElement.textContent = block.src // Mermaid rejects fenced Markdown as diagram source
