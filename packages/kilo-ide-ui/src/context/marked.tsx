...
// marked-shiki highlighted code blocks synchronously during
...
// import types for double-dollar math extension
...
import { ensureKiloDiffTheme, KILO_DIFF_THEME } from "../pierre/kilo-diff-theme"

// the "Kilo" diff/highlight theme registration moved to
...
// theme object consumed by the streaming Shiki worker
...
// double-dollar-only math rules for marked.
const BLOCK = /^\$\$\n((?:\\[^]|[^\\])+?)\n\$\$(?:\n|$)/
const INLINE = /^\$\$(?!\$)((?:\\.|[^\\\n])*?(?:\\.|[^\\\n$]))\$\$/


// isolate KaTeX from the markdown root dir=auto.
...
function renderMathInText(text: string): string {
  ...
  // removed single-dollar inline math ($...$) rendering.
...
// parseFilePath imported from ../file-path

// highlight cache for deferred highlighting
...
// expose the parser setup for Kilo markdown tests.
export const createMarkedParser = (props: { nativeParser?: NativeMarkdownParser }) => {
  // two-pass parser — first pass skips Shiki highlighting
  ...
  const parser = marked.use(
    {
      ...
      // Marked accepts a tilde preceded by an opening
    ...
    // enable only double-dollar math.
    // Single $ is far more common as a currency symbol in agent responses
    ...
    // markedShiki removed — the custom `code` renderer
