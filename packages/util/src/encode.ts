export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // devilcode_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // devilcode_change - encode as UTF-8 bytes
