...
export namespace MemoryCapture {
  export const turn = Effect.fn("MemoryCapture.turn")(function* (input: {
    ...
    // An omitted diff view means unavailable, not an assertion that the turn made no edits.
    const changed = view.diffs ? summarizeDiffs(diffs) : "unavailable"
    const substantial = view.diffs ? hasSubstantialDiff(diffs) : false
    const edited = view.diffs ? hasUserEdit(diffs) : false
    ...
    const echo = !edited && assistant.length < 1200 && view.recalledMemory === true
