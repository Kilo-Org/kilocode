...
export namespace Memory {
  ...
  function skip(input: MemoryOperations.Rejection[]) {
    return input.map((item) => (item.reason === "out_of_scope" ? { reason: item.reason } : item))
  }

  function ops(input: { ops: MemoryOperations.Op[]; skipped: MemoryOperations.Rejection[] }) {
  ...
  export async function apply(input: {
    ...
    if (trigger === "explicit") {
      await MemoryFiles.decide(input.root, {
        ...
        skipped: skip(result.skipped),
        operations: ops({ ops: inputOps, skipped: result.skipped }),
