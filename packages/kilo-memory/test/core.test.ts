...
describe("memory core package", () => {
  ...
  test("state parser uses Kilo's default-off auto mode, ignores persisted limits, and rejects non-finite stats", () => {
    ...
    expect(paused.autoConsolidate).toBe(false)
    expect(paused.verbose).toBe(false)
    expect(missing.enabled).toBe(false)
    expect(missing.autoConsolidate).toBe(false)
  ...
  test("corrupted state fails closed without changing files", async () => {
    await use(async (t) => {
      await Memory.enable({ root: t.root })
      await writeFile(MemoryPaths.files(t.root).state, "{", "utf8")

      const before = await Bun.file(MemoryPaths.files(t.root).state).text()
      const files = await readdir(t.root)

      await expect(MemoryFiles.readState(t.root)).rejects.toThrow()
      expect(await Bun.file(MemoryPaths.files(t.root).state).text()).toBe(before)
      expect(files.some((file) => file.startsWith("state.json.bad-"))).toBe(false)
    })
  })
  ...
  test("unsupported state versions fail closed without repair", async () => {
    await use(async (t) => {
      await Memory.enable({ root: t.root })
      ...
      const before = await Bun.file(MemoryPaths.files(t.root).state).text()
      const files = await readdir(t.root)
      await expect(MemoryFiles.readState(t.root)).rejects.toThrow("unsupported memory state version")
      expect(await Bun.file(MemoryPaths.files(t.root).state).text()).toBe(before)
      expect(files.some((file) => file.startsWith("state.json.bad-"))).toBe(false)
    })
  })
