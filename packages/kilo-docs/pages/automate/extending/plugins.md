...
Configure the `attention` section of `tui.json` or `tui.jsonc` to enable notifications and sounds. The former `kilo console` configuration interface is deprecated. See [CLI Notifications and Sounds](/docs/code-with-ai/platforms/cli#cli-notifications-and-sounds) for configuration and custom sound overrides.
