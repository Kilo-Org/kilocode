...
The Agent Manager tab bar supports tab actions from a right-click menu: **Pin** / **Unpin**, **Close**, **Close Others**, and **Close to the Right**. Pinned session tabs render first with a pin marker and no close button, and they survive Close Others and Close to the Right. Close to the Right closes the session, terminal, and review tabs that sit after the target; it is hidden when nothing closable is to the right, and the current selection stays where it is unless the visible tab was one of the closed tabs.
