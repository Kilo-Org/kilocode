...
{% callout type="note" title="Agent Manager Browser Is Separate" %}
The experimental **Agent Manager Browser** under **Settings → Experimental** is a different feature. It shows local application previews inside Agent Manager and does not replace Playwright browser automation. The two features have independent settings.
{% /callout %}
