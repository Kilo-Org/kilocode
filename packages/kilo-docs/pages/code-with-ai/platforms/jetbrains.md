...
- **Checkpoints** — toggle snapshots for the current project and configure machine-wide automatic session cleanup, including the retention window and manual runs. Snapshots are on by default; the toggle writes the `snapshot` key to the project's config. See [Checkpoints](/docs/code-with-ai/features/checkpoints) for how snapshots, revert notices, and cleanup work.
