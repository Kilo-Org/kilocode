...
- **`require_approval_for_config_edits`** - Require approval for edits to protected config files (default: `true`). A project value covers the project's own config files; the global value covers global config directories and config files outside the project — see [Config File Protection](/docs/getting-started/settings/auto-approving-actions#config-file-protection)
