...
| `llamaCppToolSchema` | `boolean` | Enables compatibility handling for llama.cpp servers that reject unanchored JSON Schema patterns. Kilo enables this automatically for provider IDs containing `llamacpp` or `llama-server`; set it to `true` for an arbitrarily named llama.cpp provider or `false` to disable automatic detection. |
