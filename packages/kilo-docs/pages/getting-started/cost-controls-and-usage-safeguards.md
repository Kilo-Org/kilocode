...
You can also configure a cheaper model specifically for compaction, so summarization does not consume frontier model tokens. In the VS Code extension, pick the **Compaction model** under **Settings → Models**. In `kilo.jsonc`:
