...
| **Deny** | Block this specific invocation, with optional feedback for the agent |

Selecting **Deny** opens an optional feedback field. Enter an instruction telling the agent what to do differently, then press Enter (or select **Reject**) to deny the request with that feedback so the agent can revise its proposal. Press Escape (or select **Cancel**) to return to the prompt. Leaving the field empty denies the request without feedback.
...
| **Reject** | Block this specific invocation, with optional feedback for the agent |

Selecting **Reject** opens a field where you can tell Kilo what to do differently. Enter optional instructions and press Enter to reject with that feedback so the agent can revise its proposal, or press Escape to cancel the rejection. Leaving the field empty is a plain rejection.
