...
import { parseServiceOrigin } from "../src/cloud/origin"
import { MAX_CLOUD_AGENT_RESPONSE_BYTES } from "../src/cloud/response-json"
import { createCloudAgentClient } from "../src/cloud/trpc"

// Ported from origin/main packages/opencode/test/kilocode/cloud/transport.test.ts,
// with v1 "@/kilocode/cloud/*" aliases rewritten to relative imports. Behaviour is
// exercised against a loopback Bun.serve stub only; no live endpoint is contacted.
...
describe("Cloud Agent transport", () => {
  ...
  test("surfaces non-admission HTTP failures with their status-specific message", async () => {
    const server = Bun.serve({
      hostname: "127.0.0.1",
      port: 0,
      fetch(request) {
        ...
        const raw = url.searchParams.get("input")
        const input = JSON.parse(raw ?? "null") as { messageId?: string }
        const status = input.messageId === MESSAGE ? 401 : input.messageId === OTHER_MESSAGE ? 402 : 404
    ...
    try {
      ...
      const lookup = (messageId: string) =>
        agent.getMessageResult({ cloudAgentSessionId: SESSION, messageId }).then(
          () => new Error("Expected lookup to fail"),
          (cause: unknown) => (cause instanceof Error ? cause : new Error("Non-error rejection")),
        )

      expect((await lookup(MESSAGE)).message).toBe("Cloud Agent rejected authentication")
      expect((await lookup(OTHER_MESSAGE)).message).toBe("Cloud Agent requires additional balance")
      expect((await lookup(SESSION_MESSAGE)).message).toBe("Cloud Agent session or message was not found")
    } finally {
      await server.stop(true)
    }
  })
