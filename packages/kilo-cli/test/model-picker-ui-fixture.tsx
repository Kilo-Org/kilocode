...
const gateway = Bun.serve({
  ...
  fetch(request) {
    ...
    if (path === "/api/profile") return Response.json({ organizations: [], hasPersonalAccount: true })
    if (path === "/api/openrouter/models") {
      assert.equal(request.headers.get("authorization"), "Bearer fixture-only")
      calls++
      return Response.json({
        data: [
          { id: "recommended", preferredIndex: 0 },
          { id: "kilo-auto/free", autoRouting: { models: ["recommended"] } },
        ],
      })
    }
  ...
  await Effect.runPromise(
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const metadata = yield* Effect.promise(() => client.rpc(KiloModels.Definition).list({}, { location }))
        assert.equal(metadata[0]?.recommendedIndex, 0)
        ...
        yield* Effect.tryPromise(async () => {
          ...
          await setup.waitForFrame((frame) => frame.includes("Favorites"), { maxPasses: 600 })
          setup.mockInput.pressEscape()
          await setup.mockInput.typeText("/models")
          setup.mockInput.pressEnter()
