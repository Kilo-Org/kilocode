...
test.each([false, true])(
  "headless run resolves Code and resumes without shadowing a custom agent (custom=%s)",
  async (custom) => {
    ...
    const child = Bun.spawn([process.execPath, "--no-env-file", path.join(import.meta.dir, "interactive-fixture.ts")], {
      ...
      env: {
        ...
        KILO_FIXTURE_CONFIG: JSON.stringify({
          model: "fixture/chat",
          ...(custom ? { agents: { code: { mode: "primary", description: "Project coding agent" } } } : {}),
          providers: {
            fixture: {
    ...
    try {
      ...
      expect((await client.session.get({ sessionID: first.sessionID })).agent).toBe(custom ? "code" : "build")
      ...
      expect((await client.session.get({ sessionID: second.sessionID })).agent).toBe(custom ? "code" : "build")
