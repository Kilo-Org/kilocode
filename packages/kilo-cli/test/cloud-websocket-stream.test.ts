...
import { streamAgentEvents } from "../src/cloud/websocket-stream"

// Ported from origin/main packages/opencode/test/kilocode/cloud/websocket-stream.test.ts,
// with the v1 "@/kilocode/cloud/websocket-stream" alias rewritten to a relative import.
// WebSocket is stubbed in-process; no live socket is opened.
...
describe("streamAgentEvents", () => {
  ...
  test("rejects a pre-aborted signal before opening a WebSocket", async () => {
    ...
    ).rejects.toThrow("WebSocket stream aborted")
    expect(constructed).toBe(false)
  })

  test("a live abort closes the socket and rejects", async () => {
    const closed: Array<number | undefined> = []
    const Socket = mockWebSocket([{ type: "message", data: '{"event":"one"}' }], {
      onClose: (code) => closed.push(code),
    })
    const controller = new AbortController()
    const lines: string[] = []
    const pending = streamAgentEvents({
      streamUrl: "/stream?ticket=tok",
      origin: "https://agent.example",
      writeLine: (line) => {
        lines.push(line)
      },
    ...
    // Abort after the socket is constructed but before it settles.
    queueMicrotask(() => controller.abort())
    await expect(pending).rejects.toThrow("WebSocket stream aborted")
