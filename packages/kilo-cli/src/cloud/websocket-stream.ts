...
export interface StreamAgentEventsOptions {
  ...
  // Port delta from v1: v1 offered no external cancellation (only the idle,
  // complete-grace, and close timers). This optional AbortSignal lets the CLI
  // cancel a live stream: a pre-aborted signal rejects before any WebSocket is
  // opened; a live abort closes the socket, clears every timer, and rejects.
...
export function streamAgentEvents(options: StreamAgentEventsOptions): Promise<void> {
  ...
  if (signal?.aborted) return Promise.reject(new CloudError("WebSocket stream aborted"))
  ...
  return new Promise((resolve, reject) => {
    ...
    function onAbort() {
      abort("WebSocket stream aborted")
    }
    ...
    // Register the abort listener before arming timers and before any frame can
    // arrive, then re-check for a race with an abort that landed during setup.
...
// Exported so the stream.prepare handler can validate and pin a caller-provided
// stream URL to the configured agent origin before any ticket fetch.
export function resolveWebSocketUrl(streamUrl: string, origin: string): string {
  let url: URL
  try {
