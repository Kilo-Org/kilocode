---
title: Zen
description: Curated list of models provided by Kilo CLI.
---
...
Kilo CLI Zen is a list of tested and verified models provided by the Kilo CLI team.

:::note
Kilo CLI Zen is currently in beta.
:::

Zen works like any other provider in Kilo CLI. You login to Kilo CLI Zen and get
...
:::tip
We tested a select group of models and providers that work well with Kilo CLI.
:::
...
Kilo CLI Zen is an AI gateway that gives you access to these models.

---

## How it works

Kilo CLI Zen works like any other provider in Kilo CLI.

1. You sign in to **<a href={console}>Kilo CLI Zen</a>**, add your billing
   details, and copy your API key.
2. You run the `/connect` command in the TUI, select Kilo CLI Zen, and paste your API key.
...
| Model              | Model ID           | Endpoint  FIXME                                    | AI SDK Package              |
| ------------------ | ------------------ | -------------------------------------------------- | --------------------------- |
...
The [model id](/docs/config/#models) in your Kilo CLI config
uses the format `kilo/<model-id>`. For example, for GPT 5.2 Codex, you would
use `kilo/gpt-5.2-codex` in your config.
...
```
https://kilo.ai/zen/v1/models FIXME
```
...
- Big Pickle is a stealth model that's free on Kilo CLI for a limited time. The team is using this time to collect feedback and improve the model.
...
4. Have **no lock-in** by allowing you to use it with any other coding agent. And always let you use any other provider with Kilo CLI as well.
