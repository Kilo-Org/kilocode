...
To use Kilo CLI via ACP, configure your editor to run the `kilo acp` command.
...
```json title="acp.json"
{
  "agent_servers": {
    "Kilo": {
      "command": "/absolute/path/bin/kilo",
      "args": ["acp"]
    }
  }
}
...
If you need to pass environment variables (like `KILO_API_KEY`), refer to [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) in the CodeCompanion.nvim documentation for full details.
