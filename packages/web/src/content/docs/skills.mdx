...
- Project config: `.kilo/skills/<name>/SKILL.md`
- Global config: `~/.config/kilo/skills/<name>/SKILL.md`
...
It loads any matching `skills/*/SKILL.md` in `.kilo/` and any matching `.claude/skills/*/SKILL.md` along the way.

Global definitions are also loaded from `~/.config/kilo/skills/*/SKILL.md` and `~/.claude/skills/*/SKILL.md`.
...
Create `.kilo/skills/git-release/SKILL.md` like this:
...
Control which skills agents can access using pattern-based permissions in `kilo.json`:
...
**For built-in agents** (in `kilo.json`):
