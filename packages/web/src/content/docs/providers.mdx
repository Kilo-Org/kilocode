...
Kilo CLI Zen is a list of models provided by the Kilo CLI team that have been
tested and verified to work well with Kilo CLI. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Kilo CLI Zen.
:::

1. Run the `/connect` command in the TUI, select kilo, and head to [kilo.ai/auth](https://kilo.ai/auth) FIXME.
...
It works like any other provider in Kilo CLI and is completely optional to use.
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running kilo:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY kilo

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile kilo

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX kilo
   ```
   ...
   For project-specific or persistent configuration, use `kilo.json`:
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX kilo
   ```
...
`kilo.json` file. It is also recommended to disable session sharing.
...
More documentation on [kilo-gitlab-auth](https://www.npmjs.com/package/@gitlab/kilo-gitlab-auth) FIXME homepage.

##### Configuration

Customize through `kilo.json`:
...
{
  "$schema": "https://kilo.ai/config.json" FIXME,
  "plugin": ["@gitlab/kilo-gitlab-plugin"] FIXME
}
```
...
2. Set the required environment variables:
   ...
   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id kilo
   ```
...
```jsonc title="~/.config/kilo/kilo.jsonc"
{
  "$schema": "https://kilo.ai/config.json" FIXME,
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [kilo-helicone-session](https://github.com/H2Shami/kilo-helicone-session) FIXME plugin to automatically log each Kilo CLI conversation as a session in Helicone.

```bash
npm install -g kilo-helicone-session
```
...
```json title="kilo.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://kilo.ai/config.json" FIXME,
  "provider": {
    "llama.cpp": {
...
```json title="kilo.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://kilo.ai/config.json" FIXME,
  "provider": {
    "lmstudio": {
...
```json title="kilo.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://kilo.ai/config.json" FIXME,
  "provider": {
    "ollama": {
...
Kilo CLI Zen is a list of tested and verified models provided by the Kilo CLI team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Kilo CLI Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Kilo CLI Zen**.

   ```txt
   /connect
   ```

3. Enter your Kilo CLI API key.
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' kilo
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group kilo
   ```
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in kilo.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `kilo.json` file in your project directory:

   ```json title="kilo.json" ""myprovider"" {5-15}
   {
     "$schema": "https://kilo.ai/config.json" FIXME,
     "provider": {
       "myprovider": {
...
```json title="kilo.json" {9,11,17-20}
{
  "$schema": "https://kilo.ai/config.json" FIXME,
  "provider": {
    "myprovider": {
...
2. For custom providers, check the kilo config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your kilo config.
