...
- `edit` - Cannot modify existing files, except for files located at `.kilo/plans/*.md` to detail the plan itself
...
Configure modes in your `kilo.json` config file:
...
- Global: `~/.config/kilo/modes/`
- Project: `.kilo/modes/`

```markdown title="~/.config/kilo/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Create mode files in `.kilo/modes/` for project-specific modes or `~/.config/kilo/modes/` for global modes:

```markdown title=".kilo/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/kilo/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
