...
- `.kilo/plugins/` - Project-level plugins
- `~/.config/kilo/plugins/` - Global plugins
...
```json title="kilo.json"
{
  ...
  "plugin": ["kilo-helicone-session", "kilo-wakatime", "@my-org/custom-plugin"]
}
```
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/kilo/node_modules/`.
...
1. Global config (`~/.config/kilo/kilo.json`)
2. Project config (`kilo.json`)
3. Global plugin directory (`~/.config/kilo/plugins/`)
4. Project plugin directory (`.kilo/plugins/`)
...
```ts title=".kilo/plugins/my-plugin.ts"
...
```js title=".kilo/plugins/example.js"
...
- `client`: An kilo SDK client for interacting with the AI.
...
import type { Plugin } from "@kilo-ai/plugin"
...
Here are some examples of plugins you can use to extend kilo.
...
```js title=".kilo/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "kilo"'`
      }
    },
  }
}
...
Prevent kilo from reading `.env` files:

```javascript title=".kilo/plugins/env-protection.js"
...
Plugins can also add custom tools to kilo:

```ts title=".kilo/plugins/custom-tools.ts"
import { type Plugin, tool } from "@kilo-ai/plugin"
...
```ts title=".kilo/plugins/compaction.ts"
import type { Plugin } from "@kilo-ai/plugin"
...
```ts title=".kilo/plugins/custom-compaction.ts"
import type { Plugin } from "@kilo-ai/plugin"
