...
```javascript
import { createOpencode } from "@kilo-ai/sdk"
...
You can pass a configuration object to customize behavior. The instance still picks up your `kilo.json`, but you can override or add configuration inline:

```javascript
import { createOpencode } from "@kilo-ai/sdk"

const kilo = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${kilo.server.url}`)
...
```javascript
import { createOpencodeClient } from "@kilo-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@kilo-ai/sdk"
```
