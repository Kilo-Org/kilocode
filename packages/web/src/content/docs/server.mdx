...
The `kilo serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an Kilo CLI client can use.
...
Set `KILO_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `kilo`, or set `KILO_SERVER_USERNAME` to override it. This applies to both `kilo serve` and `kilo web`.

```bash
KILO_SERVER_PASSWORD=your-password kilo serve
```
...
When you run `kilo` it starts a TUI and a server. Where the TUI is the
...
Kilo CLI TUI running, `kilo serve` will start a new server.
