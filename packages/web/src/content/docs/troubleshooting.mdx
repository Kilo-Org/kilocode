...
- **macOS/Linux**: `~/.local/share/kilo/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\kilo`
...
Kilo CLI Desktop runs a local Kilo CLI server (the `kilo-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Kilo CLI` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/kilo/kilo.jsonc` (or `~/.config/kilo/kilo.json`)
- **macOS/Linux** (older installs): `~/.local/share/kilo/kilo.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\kilo\kilo.jsonc`
...
Kilo CLI can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/kilo/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\kilo\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Kilo CLI can rebuild it.

1. Quit Kilo CLI Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/kilo`
- **Linux**: delete `~/.cache/kilo` (or run `rm -rf ~/.cache/kilo`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\kilo`

3. Restart Kilo CLI Desktop.

---

### Fix server connection issues

Kilo CLI Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `kilo.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `KILO_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `KILO_PORT` (or pick a free port) and restart.
...
On Windows, Kilo CLI Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Notifications not showing

Kilo CLI Desktop only shows system notifications when:

- notifications are enabled for Kilo CLI in your OS settings, and
...
1. Quit Kilo CLI Desktop.
2. Find and delete these files (they live in the Kilo CLI Desktop app data directory):

- `kilo.settings.dat` (desktop default server URL)
- `kilo.global.dat` and `kilo.workspace.*.dat` (UI state like recent servers/projects)
...
1. **Report issues on GitHub**
   ...
   [**github.com/anomalyco/kilo/issues**](https://github.com/anomalyco/kilo/issues) FIXME
...
2. **Join our Discord**
   ...
   [**kilo.ai/discord**](https://kilo.ai/discord) FIXME
...
To figure out what models you have access to, run `kilo models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\kilo`
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\kilo`
