...
By default, Kilo CLI uses our own `kilo` theme.
...
2. **User config directory** - Defined in `~/.config/kilo/themes/*.json` or `$XDG_CONFIG_HOME/kilo/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.kilo/themes/*.json`
4. **Current working directory** - Defined in `./.kilo/themes/*.json`
...
vim ~/.config/kilo/themes/my-theme.json
```
...
{
  "$schema": "https://kilo.ai/theme.json" FIXME,
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
