...
- Locally by placing them in the `.kilo/tools/` directory of your project.
- Or globally, by placing them in `~/.config/kilo/tools/`.
...
```ts title=".kilo/tools/database.ts" {1}
import { tool } from "@kilo-ai/plugin"
...
```ts title=".kilo/tools/project.ts" {8}
import { tool } from "@kilo-ai/plugin"
...
```python title=".kilo/tools/add.py"
import sys
...
```ts title=".kilo/tools/python-add.ts" {10}
import { tool } from "@kilo-ai/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .kilo/tools/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
