...
You can also have global rules in a `~/.config/kilo/AGENTS.md` file. This gets applied across all Kilo CLI sessions.
...
- **Global rules**: `~/.claude/CLAUDE.md` (used if no `~/.config/kilo/AGENTS.md` exists)
- **Skills**: `~/.claude/skills/` — see [Agent Skills](/docs/skills/) for details

To disable Claude Code compatibility, set one of these environment variables:

```bash
export KILO_DISABLE_CLAUDE_CODE=1        # Disable all .claude support
export KILO_DISABLE_CLAUDE_CODE_PROMPT=1 # Disable only ~/.claude/CLAUDE.md
export KILO_DISABLE_CLAUDE_CODE_SKILLS=1 # Disable only .claude/skills
```
...
2. **Global file** at `~/.config/kilo/AGENTS.md`
3. **Claude Code file** at `~/.claude/CLAUDE.md` (unless disabled)

The first matching file wins in each category. For example, if you have both `AGENTS.md` and `CLAUDE.md`, only `AGENTS.md` is used. Similarly, `~/.config/kilo/AGENTS.md` takes precedence over `~/.claude/CLAUDE.md`.

---

## Custom Instructions

You can specify custom instruction files in your `kilo.json` or the global `~/.config/kilo/kilo.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
The recommended approach is to use the `instructions` field in `kilo.json`:
...
:::tip
For monorepos or projects with shared standards, using `kilo.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
