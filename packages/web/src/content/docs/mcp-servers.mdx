...
You can define MCP servers in your [Kilo CLI Config](https://kilo.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="kilo.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/kilo` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
If the server requires authentication, Kilo CLI will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `kilo mcp auth <server-name>`.
...
```bash
kilo mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Kilo CLI will store the tokens securely in `~/.local/share/kilo/mcp-auth.json`.
...
kilo mcp debug my-oauth-server
```
