...
Kilo CLI integrates with VS Code, Cursor, or any IDE that supports a terminal. Just run `kilo` in the terminal to get started.
...
- **New Session**: Use `Cmd+Shift+Esc` (Mac) or `Ctrl+Shift+Esc` (Windows/Linux) to start a new Kilo CLI terminal session, even if one is already open. You can also click the Kilo CLI button in the UI.
...
3. Run `kilo` - the extension installs automatically
...
Search for **Kilo CLI** in the Extension Marketplace and click **Install**.
...
- Ensure you’re running `kilo` in the integrated terminal.
