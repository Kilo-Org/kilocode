export const dict = {
  ...
  "agentManager.intro.title": "并行运行代理，每个代理都在自己的分支上",
  "agentManager.intro.subtitle":
    "每个 worktree 都是此仓库在独立分支上的单独副本。代理在其中同时工作，你的本地文件夹保持不变。",
  "agentManager.intro.graph.agent": "代理",
  "agentManager.intro.graph.pr": "PR",
  "agentManager.intro.graph.conflict": "冲突",
  ...
  "agentManager.intro.stage1.text":
    '"本地"是你在 the base branch 上的常规工作区文件夹。worktree 是同一仓库在 .kilo/worktrees/<name> 中的第二个检出，位于自己的分支上。worktree 中的更改不会影响你的本地文件或当前分支。',
  "agentManager.intro.stage2.title": "分派任务",
  "agentManager.intro.stage2.text":
    "为每个任务创建一个 worktree。每个 worktree 都有自己的分支和代理会话。在新建工作树对话框中使用版本，可将同一个提示词最多以 4 种方式运行，并保留最佳结果。",
  "agentManager.intro.stage3.title": "代理并行运行",
  "agentManager.intro.stage3.text":
    "代理同时运行，不会互相覆盖文件。在各自的标签页中跟踪每个代理，检查差异，并从 worktree 的终端运行应用。",
  "agentManager.intro.stage4.title": "合并回来",
  "agentManager.intro.stage4.text":
    "打开差异面板，使用 Apply 将 worktree 的更改复制到你的本地分支。或者在会话终端中推送分支并打开 PR。如果两个 worktree 修改了同一段代码，后合并的那个会有冲突：请让该 worktree 中的代理先合并 the base branch。切勿在 worktree 中使用 git stash。",
  ...
  "agentManager.intro.create": "创建你的第一个 worktree",
