    ...
    <div class="model-preview">
      <Show when={m()}>
        {(model) => {
          ...
          const hasPricing = () => cost() && cost()!.available !== false && (cost()!.input > 0 || cost()!.output > 0)
            ...
            <>
              ...
              <div class="model-preview-header">
                <div class="model-preview-title-row">
                  ...
                  <Show when={isAuto(model()) || isFree(model()) || hasByok(model()) || isDataCollectedModel(model())}>
              ...
              <div class="model-preview-grid">
                ...
                {/* Pricing — hidden for free models, models with unavailable pricing, or models without fixed pricing */}
                <Show when={cost()?.available === false}>
                  <span class="model-preview-label">{language.t("model.preview.label.pricing") || "Pricing"}</span>
                  <span class="model-preview-value">{language.t("model.preview.value.unavailable") || "Unavailable"}</span>
                </Show>
                <Show when={cost() && cost()?.available !== false && !isFree(model()) && hasPricing()}>
