...
const AgentBehaviourTab: Component = () => {
  ...
  const renderMcpSubtab = () => {
    ...
    const statusColor = (name: string) => {
      ...
      if (s === "pending")
        return "var(--vscode-progressBar-background, var(--vscode-editorWarning-foreground, #ff9800))"
    ...
    const statusLabel = (name: string) => {
      ...
      const key = {
        ...
        pending: "settings.aboutKiloCode.status.connecting",
      }[s]
      return key ? language.t(key) : s
    }

    const isConnected = (name: string) => session.mcpStatus()[name]?.status === "connected"
    const isPending = (name: string) => session.mcpStatus()[name]?.status === "pending"
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: "4px", "align-items": "center" }}>
                        ...
                        <div onClick={(e: MouseEvent) => e.stopPropagation()}>
                          <Switch
                            checked={isConnected(name) || isPending(name)}
                            disabled={session.mcpLoading() === name || isPending(name)}
