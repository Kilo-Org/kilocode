  ...
  return (
    <div style={{ display: "flex", "flex-direction": "column", gap: "16px" }}>
      ...
      <Card>
        ...
        <Show when={provider()}>
          <For each={providerFields(provider())}>
            {(field, index) => (
              <SettingsRow
                title={`${providers.find((item) => item.value === provider())?.label ?? provider()} ${field.label}`}
                description={language.t("settings.indexing.providerField.description")}
                last={index() === providerFields(provider()).length - 1}
              >
                <TextField
                  value={providerValue(provider()!, field.key)}
                  placeholder={field.placeholder}
                  onInput={(e: InputEvent) => {
                    const target = e.currentTarget as HTMLInputElement
                    setProviderDrafts((prev) => ({ ...prev, [`${provider()!}.${field.key}`]: target.value }))
                  }}
                  onBlur={(e: FocusEvent) => {
                    const target = e.currentTarget as HTMLInputElement
                    saveProviderField(provider()!, field.key, target.value)
                  }}
                />
              </SettingsRow>
            )}
