...
export const ChatView: Component<ChatViewProps> = (props) => {
  ...
  onMount(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (props.cloud && !session.isCloudSessionHydrated(id())) return
      e.preventDefault()
      session.abort()
    }
  ...
  const canFork = (hasChat: boolean) =>
    hasChat && !props.cloud && !isSidebar() && session.status() === "idle" && !!props.onForkSession
      ...
      <div class="chat-messages-wrapper">
        <div class="chat-messages">
          <MessageList
            ...
            loadingLabel={props.cloud ? language.t("agentManager.cloud.connectingSession") : undefined}
          />
        </div>
      </div>
