 ...
 * when a model or reasoning change, or a long idle time, will resend the
 * context without cache. After a request, it reports a request cost at or
 * above the Request Cost Notice threshold.
 */
...
import { cacheReset, idleMinutes, noticeThreshold } from "../../context/cost-notice"

const TEXT = {
  model: "session.costNotice.model",
  variant: "session.costNotice.variant",
  idle: "session.costNotice.idle",
} as const
...
export const CostNotice: Component = () => {
  ...
  const reset = createMemo(() => {
    ...
    const last = msgs.findLast((msg) => msg.role === "user" && msg.model)?.model
    const done =
      session.status() === "idle" ? msgs.findLast((msg) => msg.role === "assistant")?.time?.completed : undefined
    const found = cacheReset({
      ...
      idle: done == null ? undefined : now() - done,
      ttl: idleMinutes(settings().cacheIdleNotice),
    })
    if (!found) return undefined
    const key = `${id}|${found.kind}|${next.model?.providerID}/${next.model?.modelID}|${next.variant ?? ""}|${done ?? ""}`
    if (hidden() === key) return undefined
    const tokens = new Intl.NumberFormat(language.locale(), { notation: "compact" }).format(found.tokens)
    const vars = { tokens, cost: money(found.cost), minutes: found.minutes }
    return { text: language.t(TEXT[found.kind], vars), dismiss: () => setHidden(key) }
  })
