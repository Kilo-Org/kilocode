...
export function getPromptVariant(
  ...
  if (variants.length === 0) return undefined
  const sessionValue = session ? promptVariant(store[variantKey(sel, agent, session)], variants) : undefined
  if (sessionValue) return sessionValue
  const pendingValue = promptVariant(store[variantKey(sel, agent)], variants)
  if (pendingValue) return pendingValue
  const configValue = pick(config, variants)
  if (configValue === null) return undefined
  return configValue
