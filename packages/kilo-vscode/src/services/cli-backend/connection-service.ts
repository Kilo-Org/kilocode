...
import { createKiloClient, type KiloClient } from "../../backend/index"
...
import { TerminalConnection } from "../../backend/terminal-connection"
...
export class KiloConnectionService {
  ...
  registerTerminalOrigin(panel: string, origin?: string) {
    if (origin) this.terminalOrigins.set(panel, origin)
    else this.terminalOrigins.delete(panel)
    this.terminalConnection?.register(panel, origin)
  }

  async terminalUrl(ptyID: string, directory: string, replayExited = false) {
    const client = await this.getClientAsync(directory)
    if (!this.terminalConnection) throw new Error("Terminal relay is unavailable")
    return this.terminalConnection.connect(client, ptyID, directory, replayExited)
  }
   ...
   */
  dispose(): void {
    void this.terminalConnection?.close().catch((error) => console.warn("Terminal relay cleanup failed", error))
    this.terminalConnection = undefined
    this.terminalOrigins.clear()
    this.stopHealthPoll()
  ...
  private async doConnect(workspaceDir: string): Promise<void> {
    ...
    this.config = config
    this.terminalConnection = new TerminalConnection({
      assets: vscode.Uri.joinPath(this.context.extensionUri, "dist").fsPath,
      serverUrl: config.baseUrl,
      serverPassword: config.password,
    })
    for (const [panel, origin] of this.terminalOrigins) this.terminalConnection.register(panel, origin)

    const client = createKiloClient({ client: server.client, directory: workspaceDir })
