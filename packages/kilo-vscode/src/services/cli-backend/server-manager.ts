...
export class ServerManager {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    const xdgData = clean(process.env.XDG_DATA_HOME) ?? path.join(os.homedir(), ".local", "share")
    const xdgConfig = clean(process.env.XDG_CONFIG_HOME) ?? path.join(os.homedir(), ".config")
    const xdgState = clean(process.env.XDG_STATE_HOME) ?? path.join(os.homedir(), ".local", "state")
    const xdgCache = clean(process.env.XDG_CACHE_HOME) ?? path.join(os.homedir(), ".cache")
    const data = clean(process.env.KILO_DATA_DIR) ?? path.join(xdgData, app)
    const config = clean(process.env.KILO_GLOBAL_CONFIG_DIR) ?? path.join(xdgConfig, app)
    const state = clean(process.env.KILO_STATE_DIR) ?? path.join(xdgState, app)
    const cache = clean(process.env.KILO_CACHE_DIR) ?? path.join(xdgCache, app)
    console.log("[Kilo New] ServerManager: 📂 CLI data dirs:")
    console.log("[Kilo New] ServerManager:   - data:  ", data)
    console.log("[Kilo New] ServerManager:   - config:", config)
    console.log("[Kilo New] ServerManager:   - state: ", state)
    console.log("[Kilo New] ServerManager:   - cache: ", cache)
