...
async function invalidate(
  ...
  const directories = scope === "global" ? [...new Set([dir, ...ctx.connection.getKnownDirectories()])] : [dir]
  for (const directory of directories) {
    const client = await ctx.connection.getClientAsync(directory)
    await client.config.refresh({ directory }, { throwOnError: true })
  }
}
