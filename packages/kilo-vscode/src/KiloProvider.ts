...
import type { Agent } from "./backend/index"
...
import { fetchKiloEmbeddingModelCatalog } from "./backend/embedding-models"
...
export class KiloProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
      ...
      switch (message.type) {
        case "webviewReady":
          if (typeof message.origin === "string")
            this.connectionService.registerTerminalOrigin(this.instanceId, message.origin)
  ...
  private async disposeGlobal(): Promise<void> {
    ...
    await Promise.all(
      this.connectionService.getKnownDirectories().map((directory) =>
        this.client!.config.refresh({ directory }, { throwOnError: true }),
      ),
    )
  ...
  /** Refresh config from disk while preserving running sessions and terminals. */
  private async handleReload(): Promise<void> {
    ...
    try {
      await this.client.config.refresh({ directory: dir }, { throwOnError: true })
    } catch (err) {
   ...
   */
  dispose(): void {
    this.connectionService.registerTerminalOrigin(this.instanceId)
    if (this.opts.focusContext) {
      void vscode.commands.executeCommand("setContext", this.opts.focusContext, false)
    }
