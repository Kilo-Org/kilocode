...
export class WorktreeManager {
  ...
  private async ensureCommit(): Promise<void> {
    ...
    const head = await this.git.raw(["rev-parse", "--verify", "--quiet", "HEAD^{commit}"]).catch((error: unknown) => {
      this.log(`ensureCommit: HEAD is not resolvable: ${error}`)
      return ""
    })
    if (head.trim()) return
    ...
    const any = await this.git.raw(["rev-list", "-n", "1", "--all"]).catch((error: unknown) => {
