...
interface ScriptTerminalDeps {
  ...
  /** Mint the ticket-bound terminal WebSocket URL on the shared connection. */
  connection: { terminalUrl(ptyID: string, cwd: string, replayExited: boolean): Promise<string> }
...
export class ScriptTerminalManager {
  ...
  private async url(client: KiloClient, kind: ScriptTerminalKind, ptyID: string, cwd: string): Promise<string> {
    try {
      // replayExited: script terminals attach with the full retained output so an
      // exited script still shows how far it got before the exit code landed.
      return await this.deps.connection.terminalUrl(ptyID, cwd, true)
    } catch (error) {
