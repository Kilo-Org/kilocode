...
export class WorktreePool {
  ...
  private async build(point: PoolStart, oid: string, names: string[]): Promise<PoolSlot | undefined> {
    ...
    this.refresh(slot)
    this.deps.log(`worktree pool: warmed slot ${slotPath} at ${oid}`)
    return slot
  }
  ...
  private async take(
    ...
    this.deps.log(`worktree pool: claimed slot ${slot.path} for ${branch}`)
  ...
  private async adopt(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      this.deps.log(`worktree pool: adopted slot ${slotPath}`)
    }
  }
  ...
  private async retarget(point: PoolStart, oid: string): Promise<void> {
    for (const slot of [...this.slots]) {
      ...
      this.refresh(slot)
      this.deps.log(`worktree pool: retargeted slot ${slot.path} to ${oid}`)
    }
  }
  ...
  private async readMetaFile(file: string | undefined): Promise<PoolMeta | undefined> {
    ...
    return this.attemptValue(async () => {
      const content = await fs.promises.readFile(file, "utf-8")
      return JSON.parse(content) as PoolMeta
    }, `read metadata ${file}`)
  }
