 ...
 *   - Native single-use terminal ticket URL acquisition
 *
...
export interface TerminalRoutingDeps {
  ...
  /** Mint a native ticket and route the connection through the local relay. */
  buildWsUrl(ptyID: string, directory: string): Promise<string>
...
export class TerminalRouter {
  ...
  private createManager(): TerminalManager {
    return new TerminalManager({
      ...
      buildWsUrl: (ptyID, cwd) => this.deps.buildWsUrl(ptyID, cwd),
      log: this.deps.log,
    })
  }
