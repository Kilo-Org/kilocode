...
const pending = new WeakMap<KiloClient, Map<string, Promise<void>>>()
...
export namespace VariantMigration {
  export function run(store: VariantStore | undefined, client: KiloClient | null, directory?: string): Promise<void> {
    ...
    const dir = directory ?? ""
    const tasks = pending.get(client) ?? new Map<string, Promise<void>>()
    const existing = tasks.get(dir)
    if (existing) return existing
    const task = migrate(store, client, directory)
    const done = task.finally(() => {
      if (tasks.get(dir) !== done) return
      tasks.delete(dir)
      if (tasks.size === 0) pending.delete(client)
    })
    tasks.set(dir, done)
    pending.set(client, tasks)
    return done
  }
}
