...
describe("browser element reference contract", () => {
  ...
  it("includes browser reference content only when the user sends the prompt", () => {
    ...
    expect(source).toMatch(
      /(?:const|let) message = composePromptMessage\(\{\s*review,\s*browser: browserText,\s*annotations: annotationsText,\s*draft\s*\}\)/,
    )
    expect(source).toContain("references.delete(token.key)")
  })
