...
function manager() {
  ...
  item.panel = {
    ...
    sessions: {
      inspectMemory: mock(async (sessionID) => calls.push(["inspect", sessionID])),
...
describe("AgentManagerProvider memory commands", () => {
  it("routes memory commands to the active Agent Manager session", async () => {
    ...
    await ctx.item.inspectMemory()
    await ctx.item.toggleMemory()

    expect(ctx.calls).toEqual(["ready", ["inspect", "ses_agent_manager"], "ready", ["toggle", "ses_agent_manager"]])
  })
  ...
  it("does not hang when the panel is disposed before it is ready", async () => {
    ...
    const pending = ctx.item.inspectMemory()
    ctx.dispose()
    await pending
