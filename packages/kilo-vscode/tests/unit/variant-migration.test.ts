...
describe("VariantMigration.run", () => {
  ...
  it("runs independent migrations for concurrent directories", async () => {
    await models({
    ...
    const ctx = setup({
      state: {
        "agent/code/anthropic/claude-sonnet-4": "high",
        "agent/review/anthropic/claude-sonnet-4": "low",
      },
      update: async () => {
        enter?.()
        await blocked
    ...
    const client = ctx.client as unknown as {
      app: { agents: (input?: { directory?: string }) => Promise<{ data: Agent[] }> }
    }
    client.app.agents = async (input) => {
      ctx.agentCalls.push(input)
      if (input?.directory === "/two") seen?.()
      return { data: [{ name: input?.directory === "/one" ? "code" : "review" } as Agent] }
    }

    const first = VariantMigration.run(ctx.store, ctx.client, "/one")
    await ready
    const second = VariantMigration.run(ctx.store, ctx.client, "/two")
    await secondReady

    expect(ctx.agentCalls).toEqual([{ directory: "/one" }, { directory: "/two" }])
    release?.()
    await Promise.all([first, second])
  })
