...
describe("permission message interception", () => {
  it.each(["permissionResponse", "permissionStatus"])("acknowledges a dropped %s", async (type) => {
    const messages: unknown[] = []
    const result = await interceptMessage(
      { type, permissionId: "p1", sessionID: "s1" },
      {
        before: async () => null,
    ...
    expect(messages).toEqual([
      { type: "permissionError", permissionID: "p1", ...(type === "permissionStatus" ? { retryable: false } : {}) },
    ])
  })
