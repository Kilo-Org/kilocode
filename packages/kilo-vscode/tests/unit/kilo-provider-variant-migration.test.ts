...
type Internals = {
  ...
  configBindings: {
    ...
    get: (id: string, connection: number, valid: () => boolean) => unknown
  }
...
function setup(opts: { config?: Config; fail?: Error } = {}) {
  ...
  let listener: ((state: "connecting" | "connected" | "disconnected" | "error") => void) | undefined
  ...
  const connection = {
    ...
    onStateChange: (next: (state: "connecting" | "connected" | "disconnected" | "error") => void) => {
      listener = next
      return () => undefined
    },
  ...
  internal.webview = { postMessage: async (message: unknown) => void messages.push(message) }
  ...
  return { internal, events, updates, agentCalls, connections, messages, state: () => listener }
}
...
describe("KiloProvider variant migration", () => {
  ...
  it("expires config bindings when the connection is restored", async () => {
    const ctx = setup()
    await ctx.internal.initializeConnection()
    const binding = ctx.internal.configBindings.create({
      connection: ctx.internal.connectionGeneration,
      scope: "global",
      directory: "/repo",
      target: { scope: "global", path: "/config/kilo.jsonc", revision: "one", exists: true, writable: true, raw: {} },
    })

    ctx.state()?.("connecting")
    ctx.state()?.("connected")

    expect(ctx.internal.configBindings.get(binding.id, ctx.internal.connectionGeneration, () => true)).toBeUndefined()
    expect(ctx.messages).toContainEqual({ type: "configBindingExpired", reason: "reconnected" })
  })
