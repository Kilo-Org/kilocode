...
it("mounts empty-session source markers and preserves the real composer through durable storage", async () => {
  ...
  const result = await build({
    entryPoints: [path.join(root, "tests/fixtures/annotation-prompt-storage.tsx")],
    bundle: true,
    conditions: ["browser"],
    external: ["happy-dom"],
    format: "esm",
  ...
  const file = path.join(root, `.annotation-prompt-${crypto.randomUUID()}.mjs`)
