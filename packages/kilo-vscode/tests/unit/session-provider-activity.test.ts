...
describe("SessionProvider activity", () => {
  // Bundling the fixture's whole webview graph and running it in a spawned
  // process regularly costs more than the 5s default budget, so the timeout is
  // explicit rather than left to chance.
  it(
    "covers real session activity lifecycle messages",
    async () => {
