import type { Plugin } from "@devilcode/plugin"
import { authenticateWithDeviceAuthTUI } from "./auth/device-auth-tui.js"

/**
 * Devil Gateway Authentication Plugin
 *
 * Provides device authorization flow for Devil Gateway
 ...
 */
export const DevilAuthPlugin: Plugin = async (ctx) => {
  return {
    auth: {
      provider: "kilo",
      async loader(getAuth, providerInfo) {
        ...
        if (auth.type === "api") {
          return {
            devilcodeToken: auth.key,
          }
        }

        // For OAuth auth, access token contains the Devil token
      ...
      methods: [
        {
          type: "oauth",
          label: "Devil Gateway (Device Authorization)",
          async authorize() {
...
export default DevilAuthPlugin
