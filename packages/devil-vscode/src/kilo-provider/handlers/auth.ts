/**
 * Authentication handlers — extracted from DevilProvider.
 *
...
export async function handleLogin(ctx: AuthContext, attempt: number, getAttempt: () => number): Promise<void> {
  ...
  console.log("[Devil New] DevilProvider: 🔐 Starting login flow...")
  ...
  try {
    ...
    console.log("[Devil New] DevilProvider: 🔐 Got auth URL:", auth.url)
    ...
    console.log("[Devil New] DevilProvider: 🔐 Login successful")
...
export async function handleLogout(ctx: AuthContext): Promise<void> {
  ...
  try {
    console.log("[Devil New] DevilProvider: 🚪 Logging out...")
    await ctx.client.auth.remove({ providerID: "kilo" }, { throwOnError: true })
    console.log("[Devil New] DevilProvider: 🚪 Logged out successfully")
    ...
    console.error("[Devil New] DevilProvider: ❌ Logout failed:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleSetOrganization(ctx: AuthContext, organizationId: string | null): Promise<void> {
  ...
  console.log("[Devil New] DevilProvider: Switching organization:", organizationId ?? "personal")
  try {
    await ctx.client.kilo.organization.set({ organizationId }, { throwOnError: true })
  } catch (error) {
    console.error("[Devil New] DevilProvider: Failed to switch organization:", error)
    ...
    try {
      ...
      console.error("[Devil New] DevilProvider: Failed to refresh profile after org switch error:", profileError)
    }
    return
  }
  ...
  try {
    ...
    console.error("[Devil New] DevilProvider: Failed to refresh agents after org switch:", error)
  }
}
...
export async function handleRefreshProfile(ctx: AuthContext): Promise<void> {
  ...
  console.log("[Devil New] DevilProvider: 🔄 Refreshing profile...")
