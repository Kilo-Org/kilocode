...
import {
  DEVIL_PROVIDER_ID as DEVIL_GATEWAY_ID,
  PROVIDER_PRIORITY as PROVIDER_ORDER,
  providerOrderIndex,
} from "../../../../src/shared/provider-model"

export { DEVIL_GATEWAY_ID, PROVIDER_ORDER }

export const DEVIL_AUTO_SMALL_IDS = new Set(["kilo-auto/small", "auto-small"])

export function isSmall(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return model.providerID === DEVIL_GATEWAY_ID && DEVIL_AUTO_SMALL_IDS.has(model.id)
}
...
export function stripSubProviderPrefix(name: string): string {
  ...
  if (prefix.toLowerCase() === DEVIL_GATEWAY_ID) return name
...
export function buildTriggerLabel(
  ...
  if (resolvedName) {
    if (providerID === DEVIL_GATEWAY_ID) return stripSubProviderPrefix(resolvedName)
    if (providerName) return `${providerName} / ${resolvedName}`
    return resolvedName
  }
  if (raw?.providerID && raw?.modelID) {
    return raw.providerID === DEVIL_GATEWAY_ID ? raw.modelID : `${raw.providerID} / ${raw.modelID}`
  }
