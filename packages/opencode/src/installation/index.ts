...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "kilocode/tap/kilo"])
      if (tapFormula.includes("kilo")) return "kilocode/tap/kilo"
        ...
        if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              `${yield* NpmConfig.registry(process.cwd())}/@kilocode/cli/${InstallationChannel}`,
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          case "curl":
            ...
            break
          case "npm":
            upgradeResult = yield* run(["npm", "install", "-g", `@kilocode/cli@${target}`])
            break
          case "pnpm":
            upgradeResult = yield* run(["pnpm", "install", "-g", `@kilocode/cli@${target}`])
            break
          case "bun":
            upgradeResult = yield* run(["bun", "install", "-g", `@kilocode/cli@${target}`])
            break
          case "brew": {
            const formula = yield* getBrewFormula()
            const env = { HOMEBREW_NO_AUTO_UPDATE: "1" }
            if (formula.includes("/")) {
              const tap = yield* run(["brew", "tap", "kilocode/tap"], { env })
              if (tap.code !== 0) {
                upgradeResult = tap
                break
              }
              const repo = yield* text(["brew", "--repo", "kilocode/tap"])
              const dir = repo.trim()
              if (dir) {
          ...
          case "choco":
            upgradeResult = yield* run(["choco", "upgrade", "kilo", `--version=${target}`, "-y"])
            break
          case "scoop":
            upgradeResult = yield* run(["scoop", "install", `kilo@${target}`])
            break
          default:
