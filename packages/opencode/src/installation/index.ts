...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "kilocode/tap/kilo"]) // kilocode_change
      if (tapFormula.includes("kilo")) return "kilocode/tap/kilo" // kilocode_change
      const coreFormula = yield* text(["brew", "list", "--formula", "kilo"]) // kilocode_change
      if (coreFormula.includes("kilo")) return "kilo" // kilocode_change
      return "kilo" // kilocode_change
    })
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        ...
        for (const check of checks) {
          ...
          const installedName = // kilocode_change
        ...
        if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              `${yield* NpmConfig.registry(process.cwd())}/@kilocode/cli/${InstallationChannel}`, // kilocode_change
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "brew": {
            ...
            if (formula.includes("/")) {
              const tap = yield* run(["brew", "tap", "kilocode/tap"], { env }) // kilocode_change
              if (tap.code !== 0) {
                upgradeResult = tap
                break
              }
              const repo = yield* text(["brew", "--repo", "kilocode/tap"]) // kilocode_change
              const dir = repo.trim()
              if (dir) {
