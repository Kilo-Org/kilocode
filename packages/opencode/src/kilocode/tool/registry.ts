...
import { AgentManagerInspectTool } from "./agent-manager-inspect"
...
export namespace KiloToolRegistry {
  ...
  export function infos() {
    return Effect.gen(function* () {
      ...
      const inspect = yield* AgentManagerInspectTool
      return { codebase, recall, manager, inspect }
    })
  }
  ...
  export function build(
    tools: { codebase: Tool.Info; recall: Tool.Info; manager: Tool.Info; inspect: Tool.Info },
    deps: Deps,
  ) {
    return Effect.gen(function* () {
      const base = yield* Effect.all({
        ...
        inspect: Tool.init(tools.inspect),
      })
  ...
  export function extra(
    tools: { codebase: Tool.Def; semantic?: Tool.Def; recall: Tool.Def; manager: Tool.Def; inspect: Tool.Def },
    ...
    return [
      ...
      ...(Flag.KILO_CLIENT === "vscode" && cfg.experimental?.agent_manager_tool === true
        ? [tools.manager, tools.inspect]
        : []),
    ]
  }
}
