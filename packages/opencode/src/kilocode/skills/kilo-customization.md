...
For config precedence or the full config-directory search order, load `skill({ name: "kilo-config" })` when needed. For permission rules beyond the agent example below, load `skill({ name: "kilo-tools" })`.
