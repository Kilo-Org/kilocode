# Kilo Tool Permissions and MCP Servers

These settings belong in `kilo.json` (or `kilo.jsonc`). If the config location or precedence is unclear, load `skill({ name: "kilo-config" })`.

## Permissions

Scalar form applies to all patterns. Object form maps glob patterns to actions. Evaluated top-to-bottom; the last matching rule wins. Put broad patterns first, then specific overrides after.
