...
For these tasks, use the `skill` tool to load only the matching guide. These are separately bundled skills, not files relative to this guide; do not load all of them for a general config question.

- Custom commands, agents, skills, or legacy workflows: `skill({ name: "kilo-customization" })`.
- Tool permissions or MCP server connections: `skill({ name: "kilo-tools" })`.
- Agent Manager setup/run scripts, worktree integration, conflicts, or state recovery: `skill({ name: "kilo-agent-manager" })`.
- CLI keybinds, themes, slash commands, or interactive display/session controls: `skill({ name: "kilo-tui" })`.
