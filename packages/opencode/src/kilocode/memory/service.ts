...
import { skipLine, type CaptureSkip } from "@kilocode/kilo-memory/capture"
import type { Memory } from "@kilocode/kilo-memory/memory"
...
export namespace MemoryService {
  ...
  export interface Interface {
    ...
    readonly disable: (input: KiloMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof KiloMemory.disable>>, Failure>
    readonly rebuild: (input: KiloMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof KiloMemory.rebuild>>, Failure>
    readonly configure: (input: ConfigureInput) => Effect.Effect<Awaited<ReturnType<typeof KiloMemory.configure>>, Failure>
    ...
    readonly session: (input: SessionInput) => Effect.Effect<Awaited<ReturnType<typeof MemoryFiles.readSession>>, Failure>
    readonly sources: (input: RootInput) => Effect.Effect<Sources, Failure>
    readonly recent: (input: RecentInput) => Effect.Effect<Awaited<ReturnType<typeof MemoryFiles.recentSessions>>, Failure>
          ...
          MemoryFiles.queue(input.root, async () => {
            ...
            await MemoryFiles.writeState(input.root, {
              ...
              stats: {
                ...
                lastConsolidationCost: input.cost ?? state.stats.lastConsolidationCost, // kilocode_change - keep prior cost when unknown, don't overwrite with 0
