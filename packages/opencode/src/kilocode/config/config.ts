...
export namespace KilocodeConfig {
  ...
  export async function loadLegacyConfigs(input: {
    ...
    let rulesModes: string[] | undefined
    ...
    try {
      ...
      if (Object.keys(migration.agents).length > 0) {
        const modes = Object.keys(migration.agents)
        rulesModes = [...RulesMigrator.KNOWN_MODES, ...modes]
    ...
    // Load Kilocode rules
    try {
      const migration = await RulesMigrator.migrate({ projectDir: input.projectDir, modes: rulesModes })
      ...
      if (Object.keys(migration.modeInstructions).length > 0) {
        result = input.merge(result, { modeInstructions: migration.modeInstructions })
        log.debug("loaded kilocode mode-specific rules", {
          modes: Object.keys(migration.modeInstructions),
        })
      }
