...
async function resolveWorktree(name: string, root: string, timeoutMs = 10 * 60_000) {
  ...
  return withInstance(root, async (run) => {
    ...
    if (found && (await Filesystem.exists(found.directory))) {
    ...
    UI.println(`Worktree ready at ${info.directory}`)
...
async function resolveSessionWorktree(sessionID: string, fallback: string) {
  try {
    ...
    const session = await AppRuntime.runPromise(
      Session.Service.use((svc) => svc.get(Schema.decodeUnknownSync(SessionID)(sessionID))),
    )
    if (!session.directory || session.directory === fallback) return fallback
    if (!(await Filesystem.exists(session.directory))) return fallback
    UI.println(`Resuming session in its original worktree: ${session.directory}`)
