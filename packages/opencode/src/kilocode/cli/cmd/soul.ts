// kilo soul — constitution layer commands.
//   kilo soul validate            load SOUL.md + suite for this project and run the entry lint
//   kilo soul eval --suite S --responses R [--baseline B] [--report report.md]
...
import { Soul } from "@/kilocode/soul/soul"
import { SoulEval } from "@/kilocode/soul/eval"
...
function findSoulFile(startDir: string): string | undefined {
  ...
  const globalCandidate = path.join(Global.Path.config, Soul.FILENAME)
...
export const SoulValidateCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.soul.validate")(function* () {
    ...
    if (!filepath)
      return yield* fail(`no ${Soul.FILENAME} found (looked up from ${directory} and ${Global.Path.config})`)
    ...
    if (suiteFile.size === 0)
      return yield* fail(`soul: eval suite not found at ${soul.suitePath}; every soul needs a paired probe suite`, 2)
    ...
    console.log(`soul valid: ${filepath} (${soul.axioms.length} axioms, suite ${soul.suitePath})`)
  }),
})
