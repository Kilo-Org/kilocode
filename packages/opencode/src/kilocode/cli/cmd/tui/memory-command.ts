...
export async function runMemoryCommand(input: {
  ...
  try {
    ...
    if (parsed.operation === "enable") {
      ...
      input.toast.show({
        variant: "success",
        message: `${name} enabled (${tokens(result.index.tokens)}). Kilo can inject saved project context and auto-save durable project facts from completed turns. Auto-save is local, project-scoped, best-effort redacted, audited, and can be disabled with /memory auto off.`,
      })
      return true
    }
    if (parsed.operation === "auto") {
      const result =
        parsed.mode === "status"
          ? read(await input.client.memory.status(route(input.workspace)))
          : read(
              await input.client.memory.configure({
                ...route(input.workspace),
    ...
    if (parsed.operation === "purge") {
      read(await input.client.memory.purge({ ...route(input.workspace), confirm: true }))
