...
export namespace WorktreeDiff {
  ...
  async function list(dir: string, ancestor: string, log: Log.Logger): Promise<Meta[]> {
    ...
    for (const line of nameStatus.stdout.toString().trim().split("\n")) {
      ...
      const counts = stat.get(file) ?? { additions: 0, deletions: 0, binary: false }
      result.push({
        file,
  ...
  async function readBefore(dir: string, ancestor: string, file: string, status: Status, binary: boolean) {
    if (status === "added" || binary) return ""
    const result = await $`git show ${ancestor}:${file}`.cwd(dir).quiet().nothrow()
    return result.exitCode === 0 ? result.stdout.toString() : ""
  }

  async function readAfter(dir: string, file: string, status: Status, binary: boolean) {
    if (status === "deleted" || binary) return ""
  ...
  async function load(dir: string, ancestor: string, meta: Meta): Promise<Item> {
    const before = await readBefore(dir, ancestor, meta.file, meta.status, meta.binary)
    const after = await readAfter(dir, meta.file, meta.status, meta.binary)
    const additions =
      meta.status === "added" && meta.additions === 0 && !meta.tracked && !meta.binary ? lines(after) : meta.additions
    return {
      file: meta.file,
      before,
      after,
      ...
      binary: meta.binary ? true : undefined,
      status: meta.status,
  ...
  export async function statsOnly(input: { dir: string; base: string; log?: Log.Logger }) {
  ...
  export async function full(input: { dir: string; base: string; log?: Log.Logger }) {
    ...
    for (let i = 0; i < items.length; i += concurrency) {
      ...
      result.push(...(await Promise.all(batch.map((item) => load(input.dir, ancestorHash, item)))))
    }
