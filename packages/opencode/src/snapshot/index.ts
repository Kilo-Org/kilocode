...
import * as DevilSnapshot from "../devilcode/snapshot" // devilcode_change
import { Lock } from "../util/lock" // devilcode_change

export namespace Snapshot {
  const log = Log.create({ service: "snapshot" })
  const hour = 60 * 60 * 1000
  const prune = "7.days"
  export const MAX_DIFF_SIZE = 256 * 1024 // devilcode_change
  const MAX_SNAPSHOT_FILE_SIZE = 2 * 1024 * 1024 // devilcode_change — skip files >2MB during snapshot add
  ...
  export async function cleanup() {
    if (Instance.project.vcs !== "git" || Flag.DEVIL_CLIENT === "acp") return
  ...
  export async function track() {
    ...
    const cfg = await Config.get()
    if (cfg.snapshot === false) return
    const git = await DevilSnapshot.prepare() // devilcode_change
    using _lock = await Lock.write(git) // devilcode_change
    await add(git)
  ...
  // devilcode_change start — batched revert: group up to 100 files per git checkout (port of upstream #20564)
  ...
  function gitdir() {
    return DevilSnapshot.gitdir() // devilcode_change
  }
