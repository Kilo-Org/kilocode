...
export class CodeIndexServiceFactory {
  ...
  public createEmbedder(): IEmbedder {
    ...
    if (provider === "ollama") {
      ...
      return new CodeIndexOllamaEmbedder(config.ollamaOptions.baseUrl, config.modelId)
    }
  ...
  public createVectorStore(): IVectorStore {
    ...
    if (!vectorSize || vectorSize <= 0) {
      throw new Error(
        `Cannot determine vector dimension for model "${modelId}" with provider "${provider}". ` +
    ...
    if (config.vectorStoreProvider === "lancedb") {
      const dbDir = config.lancedbVectorStoreDirectoryPlaceholder
      if (!dbDir) throw new Error("LanceDB directory path is required.")
      ...
      return new LanceDBVectorStore(this.workspacePath, vectorSize, dbDir)
    }
