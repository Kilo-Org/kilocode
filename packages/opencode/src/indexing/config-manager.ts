...
export class CodeIndexConfigManager {
  ...
  private applyInput(input: IndexingConfigInput): void {
    ...
    this.ollamaOptions = input.ollamaBaseUrl ? { baseUrl: input.ollamaBaseUrl, modelId: input.modelId } : undefined
  ...
  doesConfigChangeRequireRestart(prev: PreviousConfigSnapshot): boolean {
    ...
    // Vector dimension changes from model switch
    if (this.hasVectorDimensionChanged(prevProvider, prev.modelId)) return true

    return false
  }

  private hasVectorDimensionChanged(prevProvider: EmbedderProvider, prevModelId?: string): boolean {
    const currentModelId = this.modelId ?? getDefaultModelId(this.embedderProvider)
    const resolvedPrev = prevModelId ?? getDefaultModelId(prevProvider)

    if (prevProvider === this.embedderProvider && resolvedPrev === currentModelId) return false

    const prevDim = getModelDimension(prevProvider, resolvedPrev)
    const curDim = getModelDimension(this.embedderProvider, currentModelId)

    if (prevDim === undefined || curDim === undefined) return true
    return prevDim !== curDim
  }
