...
export class LanceDBVectorStore implements IVectorStore {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      if (needsRecreation) {
        ...
        log.info("LanceDB store reinitialized for vector size change", {
          workspacePath: this.workspacePath,
          dbPath: this.dbPath,
          created: true,
          vectorSize: this.vectorSize,
      ...
      throw new Error(`Failed to initialize LanceDB store: ${(error as Error).message}`)
    }
  }
  ...
  async markIndexingComplete(): Promise<void> {
    try {
      const db = await this.getDb()
      const metadataTable = await db.openTable(this.metadataTableName)
      await metadataTable.update({ values: { value: true }, where: "key = 'indexing_complete'" })
  ...
  async markIndexingIncomplete(): Promise<void> {
    try {
      const db = await this.getDb()
      const metadataTable = await db.openTable(this.metadataTableName)
      await metadataTable.update({ values: { value: false }, where: "key = 'indexing_complete'" })
