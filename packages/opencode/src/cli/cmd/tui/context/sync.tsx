...
import type { Path } from "@devilcode/sdk" // devilcode_change
import type { Workspace } from "@devilcode/sdk/v2" // devilcode_change
import { useToast } from "../ui/toast" // devilcode_change
...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    const [store, setStore] = createStore<{
      ...
      session_diff: {
        [sessionID: string]: Omit<Snapshot.FileDiff, "before" | "after">[] // devilcode_change
      }
      todo: {
    ...
    const fullSyncedSessions = new Set<string>() // devilcode_change
    ...
    async function bootstrap() {
      // devilcode_change - audit N6: removed console.log "bootstrapping" stdout noise.
        ...
        async sync(sessionID: string) {
          ...
          setStore(
            produce((draft) => {
              ...
              draft.message[sessionID] = messages.data!.map((x) => strip(x.info)) // devilcode_change
