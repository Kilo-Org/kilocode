...
// import { ShareNext } from "@/share/share-next" // kilocode_change - imports use the Kilo session ingest endpoint, not ShareNext
import { EOL } from "os"
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, projectID: string) {
  // const share = yield* ShareNext.Service // kilocode_change - Kilo's URL branch fetches from the Kilo session ingest endpoint, not ShareNext
  ...
  if (isUrl) {
    // kilocode_change start - fetch from Kilo session ingest endpoint instead of ShareNext;
    // ingest already returns the folded { info, messages } shape so we skip transformShareData.
  ...
  yield* Effect.promise(() => bootstrapImportedSessionIngest(exportData!.info.id)) // kilocode_change
