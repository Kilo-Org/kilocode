...
export const SessionListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.session.list")(function* (args) {
    const sessions = args.all // kilocode_change start - support --all/--search
      ? [...Session.listGlobal({ roots: true, limit: args.maxCount, search: args.search })]
      : // kilocode_change end
        yield* Session.Service.use((svc) =>
          svc.list({ roots: true, limit: args.maxCount, search: args.search /* kilocode_change */ }),
        )

    if (sessions.length === 0) return

    // kilocode_change start - --all uses formatGlobalSession* variants
