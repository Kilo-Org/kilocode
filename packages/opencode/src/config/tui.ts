...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.DEVIL_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.DEVIL_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.DEVIL_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".kilo") && !dir.endsWith(".opencode") && dir !== Flag.DEVIL_CONFIG_DIR) continue // devilcode_change
