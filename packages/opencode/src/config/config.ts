...
export const Info = Schema.Struct({
  ...
  // kilocode_change start
  modeInstructions: Schema.optional(Schema.Record(Schema.String, Schema.mutable(Schema.Array(Schema.String)))).annotate({
    description: "Mode-specific instruction files, keyed by agent/mode name",
  }),
  // kilocode_change end
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      let result: Info = {}
      result = mergeConfigConcatArrays(result, yield* loadFile(path.join(Global.Path.config, "config.json")))
      // kilocode_change start
      result = mergeConfigConcatArrays(result, yield* loadFile(path.join(Global.Path.config, "kilo.json")))
      result = mergeConfigConcatArrays(result, yield* loadFile(path.join(Global.Path.config, "kilo.jsonc")))
      // kilocode_change end
      result = mergeConfigConcatArrays(result, yield* loadFile(path.join(Global.Path.config, "opencode.json")))
      result = mergeConfigConcatArrays(result, yield* loadFile(path.join(Global.Path.config, "opencode.jsonc")))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result = mergeConfigConcatArrays(result, rest)
