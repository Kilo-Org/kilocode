  ...
  Effect.gen(function* () {
    ...
    const ops = Effect.fn("SessionPrompt.ops")(function* () {
      ...
      return {
        ...
        background: (input) =>
          Effect.sync(() => {
            run.fork(background(input).pipe(Effect.tapCause((cause) => Effect.logError("background subagent fiber failed", cause))))
          }),
        // kilocode_change end
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
      ...
      const context = (args: any, options: ToolExecutionOptions): Tool.Context => ({
        ...
        ask: (req) =>
          // kilocode_change start - refresh agent/session permissions for active tool calls
    ...
    const prompt: (input: PromptInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.prompt")(
      function* (input: PromptInput) {
        ...
        if (input.noReply === true && current.type === "busy") yield* waitIdle(input.sessionID)
        // kilocode_change end
    ...
    const waitIdle: (sessionID: SessionID) => Effect.Effect<void> = Effect.fn("SessionPrompt.waitIdle")(function* (
      sessionID: SessionID,
    ) {
