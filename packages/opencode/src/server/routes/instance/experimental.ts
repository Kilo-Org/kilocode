  ...
  new Hono()
    ...
    .get(
      ...
      async (c) => {
        ...
        const projectID = query.worktrees && !query.projectID ? ctx.project.id : query.projectID
        const directories = query.worktrees
          ? await runRequest(
              "ExperimentalRoutes.session.worktrees",
              c,
              Project.Service.use((svc) => WorktreeFamily.list(svc, ctx)),
            )
          : undefined
