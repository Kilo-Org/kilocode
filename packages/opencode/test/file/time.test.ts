    ...
    test("skips check when OPENCODE_DISABLE_FILETIME_CHECK is true", async () => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const original = Flag.DEVIL_DISABLE_FILETIME_CHECK
          ;(Flag as { DEVIL_DISABLE_FILETIME_CHECK: boolean }).DEVIL_DISABLE_FILETIME_CHECK = true

          try {
            // Should not throw even though file wasn't read
            await FileTime.assert(sessionID, filepath)
          } finally {
            ;(Flag as { DEVIL_DISABLE_FILETIME_CHECK: boolean }).DEVIL_DISABLE_FILETIME_CHECK = original
          }
        },
      })
    })
