...
describe("guardPermissions", () => {
  ...
  describe("non-mode agents", () => {
    ...
    for (const name of agents) {
      test(`${name} merges session rules with agent deny rules only`, () => {
        ...
        const result = guardPermissions({ agent, session })
        // writeAllow should not be included (not a deny rule)
  ...
  test("session rules come before agent rules in merge order", () => {
    ...
    const result = guardPermissions({ agent, session })
    // First rule is session deny, second is agent allow — merge is flat concat,
    // so last rule wins (Permission.ask resolves using `resolve`).
    expect(result[0].action).toBe("deny")
    expect(result[1].action).toBe("allow")
  })

  test("subagent-like agents (e.g. 'code') get only deny rules from agent config", () => {
    ...
    const result = guardPermissions({ agent, session })
    // grepAllow should NOT be included; globDeny and edit deny should be
