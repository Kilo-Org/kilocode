...
import { generateHelp, generateCommandTable } from "../../src/devilcode/help"
...
import { HelpCommand } from "../../src/devilcode/help-command"
...
// Synthetic entry for the yargs built-in .completion() command (mirrors commands.ts).
...
describe("commands.ts stays in sync with index.ts", () => {
  test("every .command() in index.ts has an entry in the commands array", async () => {
    ...
    const barrel = await Bun.file(path.resolve(import.meta.dir, "../../src/devilcode/commands.ts")).text()
