  ...
  test("merges overrides over defaults", () => {
    const info = AutonomousConfig.resolve({
      autonomous_goal: {
        ...
        routing: { local_coder_max_complexity: 1, planner_cloud: false },
        steps: { worker: 30 },
        checks: ["bun test"],
      },
    })
    ...
    expect(info.routing).toEqual({ local_small_max_complexity: 0, local_coder_max_complexity: 1, planner_cloud: false })
    expect(info.steps).toEqual({ ...AutonomousConfig.defaults.steps, worker: 30 })
