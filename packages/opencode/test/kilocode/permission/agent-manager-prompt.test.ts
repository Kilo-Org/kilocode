...
describe("Agent Manager side-effect permissions", () => {
  test("requires consent despite a broad Agent Manager allow rule", () => {
    ...
    expect(Permission.resolve("agent_manager", "delete", broad).action).toBe("ask")
  ...
  test("requires consent despite a global allow rule", () => {
    ...
    expect(Permission.resolve("agent_manager", "delete", rules).action).toBe("ask")
  })
  ...
  test("requires consent despite a saved wildcard approval", () => {
    ...
    expect(Permission.resolve("agent_manager", "delete", rules, saved).action).toBe("ask")
  })

  test("allows only explicit side-effect approvals", () => {
    const rules = Permission.fromConfig({ agent_manager: { prompt: "allow", stop: "allow", move: "allow", delete: "allow" } })
    ...
    expect(Permission.resolve("agent_manager", "delete", rules).action).toBe("allow")
  })
})
