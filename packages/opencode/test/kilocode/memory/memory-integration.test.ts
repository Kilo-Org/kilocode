...
describe("KiloMemory integration", () => {
  ...
  test("environment prompt injects non-empty memory with token metadata", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".kilo"), async () => {
      ...
      expect(text).toContain("specific relevant entry is too abbreviated")
      expect(text).toContain("Memory recall is not a prerequisite")
      expect(text).toContain("after a miss only if the historical detail is still needed")
      expect(text).not.toContain("When a request could depend on durable typed memory categories")
