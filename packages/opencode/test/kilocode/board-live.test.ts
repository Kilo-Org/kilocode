...
for (const flag of [undefined, false, true]) {
  it.live(`exposes board guidance and tools when the configured flag is ${flag}`, () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        expect(names.includes("board_read")).toBe(true)
        expect(names.includes("board_post")).toBe(true)
        expect(JSON.stringify(tools).includes("Cursor from your last board_read, not an ID from board_post")).toBe(true)
        expect(JSON.stringify(tools).includes("main is the board root, not necessarily your parent")).toBe(true)
      }),
      { config: (url) => ({ ...config(url), experimental: { shared_agent_board: flag } }) },
    ),
  )
}
