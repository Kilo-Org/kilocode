...
import { IgnoreMigrator } from "../../src/devilcode/ignore-migrator"
...
# Comment 2`
  ...
  *.env  `
  ...
  describe("migrate", () => {
    test("returns empty permission for project without .devilcodeignore", async () => {
    ...
    test("loads project .devilcodeignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("handles complex .devilcodeignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".devilcodeignore"),
            `# Secrets
secrets/
*.env
!.env.example
...
`,
  ...
  describe("loadIgnoreConfig", () => {
    test("returns empty object for project without .devilcodeignore", async () => {
      await using tmp = await tmpdir()
    ...
    test("returns permission config for project with .devilcodeignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".devilcodeignore"), "secrets/")
        },
