...
function permission(items: Permission.Request[]) {
  return {
    list: async () => items,
    reply: async () => true,
  }
...
describe("RemoteSender", () => {
  ...
  test("permission_respond uses injected permission service", async () => {
    const { conn, sent } = fakeConn()
    const replies: Permission.ReplyInput[] = []
    const sender = RemoteSender.create({
      conn,
      directory: "/tmp/test",
      log: nolog,
    ...
    sender.handle({
      ...
      data: { requestID: "permission_1", reply: "once" },
    })

    await new Promise((r) => setTimeout(r, 10))

    expect(replies).toHaveLength(1)
    expect(replies[0]?.requestID).toBe(PermissionID.make("permission_1"))
