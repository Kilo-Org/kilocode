...
describe("indexing startup degradation", () => {
  ...
  test(
    ...
    async () => {
      ...
      const on = (data: {
        ...
        if (!payload || payload.type !== KiloIndexing.Event.type) return
        if (payload.properties?.status) events.push(payload.properties.status)
      }
  ...
  test(
    "keeps degraded indexing queryable but unavailable",
    async () => {
      const init = spyOn(CodeIndexManager.prototype, "initialize").mockRejectedValue(error)
