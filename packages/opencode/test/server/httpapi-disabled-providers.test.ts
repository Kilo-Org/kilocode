...
describe("disabled-providers HttpApi", () => {
  ...
  it.instance(
    "includes disabled custom providers declared under provider",
    Effect.gen(function* () {
      const directory = (yield* TestInstance).directory
      const headers = { "x-kilo-directory": directory }
    ...
    }),
    {
      ...
      config: {
        formatter: false,
        lsp: false,
        disabled_providers: ["anthropic", "my-custom"],
        provider: {
          "my-custom": { name: "My Custom", npm: "@ai-sdk/openai-compatible", models: {} },
        },
      },
    },
  )
