...
afterEach(async () => {
  ...
  delete process.env.DEVIL_TUI_CONFIG
...
test("project config takes precedence over DEVIL_TUI_CONFIG (matches DEVIL_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.DEVIL_TUI_CONFIG = custom
    },
  })
...
test("DEVIL_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from DEVIL_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.DEVIL_CONFIG = path.join(customDir, "opencode.json")
    },
  })
