...
description: "Connect a GitHub account to your DevilClaw agent for repository access"
---

# GitHub Integration

Connect a GitHub account to your DevilClaw agent so it can clone repositories, push commits, open pull requests, and leave code reviews — all autonomously.
...
DevilClaw uses a [fine grained Personal Access Token](https://github.com/settings/tokens?type=beta) to authenticate as your bot. When creating the token, use these settings:

| Setting               | Recommended Value                         |
| --------------------- | ----------------------------------------- |
| **Token name**        | `DevilClaw-bot` (or any descriptive name)  |
...
### Step 3: Enter credentials in DevilClaw
...
DevilClaw accepts both GitHub token formats:
...
When your instance starts, DevilClaw automatically:
...
- Tokens are encrypted at rest using DevilClaw's secret management system
