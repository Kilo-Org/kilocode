...
The devil Code **memory bank** feature has been deprecated in favor of AGENTS.md.
...
1. Examine the contents in `.devilcode/rules/memory-bank/`
2. Move that content into your project's `AGENTS.md` file (or ask devil to do it for you)
   {% /callout %}

## What is AGENTS.md?

AGENTS.md is an open standard for configuring AI agent behavior in software projects. It's a simple Markdown file placed at the root of your project that contains instructions for AI coding assistants. The standard is supported by multiple AI coding tools, including devil Code, Cursor, and Windsurf.
...
When working in a subdirectory, devil Code will load both the root AGENTS.md and any subdirectory AGENTS.md files, with subdirectory files taking precedence for conflicting instructions.

## File Protection

Both `AGENTS.md` and `AGENT.md` are **write-protected files** in devil Code. This means:
...
## How AGENTS.md Works in devil Code

### Loading Behavior

When you start a task in devil Code:

1. devil Code checks for `AGENTS.md` or `AGENT.md` at the project root
...
| **[Instructions](/docs/customize/custom-rules)** | Project   | `instructions` key in project `devil.jsonc` | 2                |
| **AGENTS.md**                                    | Project   | `AGENTS.md` at project root                | 3                |
| **[Instructions](/docs/customize/custom-rules)** | Global    | `instructions` key in global `devil.jsonc`  | 4                |
| **[Skills](/docs/customize/skills)**             | Both      | `.devil/skills/`, config `skills` key       | Loaded on demand |
...
AGENTS.md works alongside devil Code's other configuration systems:

| Feature                                                        | Scope   | Location                  | Purpose                                   | Priority    |
| -------------------------------------------------------------- | ------- | ------------------------- | ----------------------------------------- | ----------- |
| **[Mode-specific Custom Rules](/docs/customize/custom-rules)** | Project | `.devilcode/rules-{mode}/` | Mode-specific rules and constraints       | 1 (Highest) |
| **[Custom Rules](/docs/customize/custom-rules)**               | Project | `.devilcode/rules/`        | devil Code-specific rules and constraints  | 2           |
| **[AGENTS.md](/docs/customize/agents-md)**                     | Project | `AGENTS.md`               | Universal standard for any AI coding tool | 3           |
| **[Global Custom Rules](/docs/customize/custom-rules)**        | Global  | `~/.devilcode/rules/`      | Global devil Code rules                    | 4           |
...
```bash
export Devil_DISABLE_EXTERNAL_SKILLS=true
```
...
- **[Custom Rules](/docs/customize/custom-rules)** - devil Code-specific rules with more control
