---
title: "Skills"
description: "Extend devil Code capabilities with skills"
---

# Skills

devil Code implements [Agent Skills](https://agentskills.io/), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when devil Code initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.devil` directory within your Home directory:

- Mac and Linux: `~/.devil/skills/`
- Windows: `\Users\<yourUser>\.devil\skills\`
...
Located in `.devil/skills/` within your project:
...
You can configure extra skill locations and remote skill URLs in your `devil.jsonc` config (project or global):
...
Global skills are located in the `.devilcode` directory within your Home directory.

- Mac and Linux: `~/.devilcode/skills/`
- Windows: `\Users\<yourUser>\.devilcode\`
...
Located in `.devilcode/skills/` within your project:
...
```bash
# For Code mode only
mkdir -p ~/.devilcode/skills-code/typescript-patterns

# For Architect mode only
mkdir -p ~/.devilcode/skills-architect/microservices
```
...
When multiple skills share the same name, project-level skills (`.devil/skills/`) take precedence over global skills (`~/.devil/skills/`). Skills from compatibility directories (`.claude/skills/`, `.agents/skills/`) and additional configured paths are loaded alongside project and global skills.
...
When multiple skills share the same name, devil Code uses these priority rules:
...
- In the **CLI**: Skills are loaded when you start a new session or run `devil run`
...
Skills are discovered when devil Code initializes:

- When VSCode starts
- When you reload the VSCode window (`Cmd+Shift+P` → "Developer: Reload Window")

Skills directories are monitored for changes to `SKILL.md` files. However, the most reliable way to pick up new skills is to reload VS or the devil Code extension.
...
In devil Code, the `name` field **must match** the parent directory name:
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.devil/skills/api-design
   ```
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.devilcode/skills/api-design
   ```
...
- **[devil Marketplace repository](https://github.com/Devil-Org/devil-marketplace)** — Browse community skills on GitHub and manually download them into your skills directory
...
- **devil Marketplace** — Browse skills directly in the devil Code extension via the Marketplace tab, or explore the [devil Marketplace repository](https://github.com/Devil-Org/devil-marketplace) on GitHub
- [Agent Skills Specification](https://agentskills.io/home) — The open specification that skills follow, enabling interoperability across different AI agents
...
3. **Check file location**: Ensure `SKILL.md` is directly inside the skill directory (e.g., `.devil/skills/my-skill/SKILL.md`), not nested further.

4. **Check config paths**: If using `skills.paths` or `skills.urls`, verify the paths and URLs are correct in your `devil.jsonc`.
...
1. **Check the Output panel**: Open `View` → `Output` → Select "devil Code" from dropdown. Look for skill-related errors.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [devil Marketplace](https://github.com/Devil-Org/devil-marketplace)!

{% tabs %}
{% tab label="VSCode" %}

While the new platform does not yet have a built-in marketplace UI, skills from the [devil Marketplace repository](https://github.com/Devil-Org/devil-marketplace) can be manually downloaded into your `.devil/skills/` directory or loaded via `skills.urls` in config.
...
3. **Fork the marketplace repository**: Visit [github.com/Devil-Org/devil-marketplace](https://github.com/Devil-Org/devil-marketplace) and create a fork
...
- Follow the [contribution guidelines](https://github.com/Devil-Org/devil-marketplace/blob/main/CONTRIBUTING.md) in the marketplace repository

For more details on contributing to devil Code, see the [Contributing Guide](/docs/contributing).
