...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between devil Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (devil Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as devil Code
...
- Single-client scenarios (one devil Code instance per server)
...
1. The client (devil Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from devil Code
...
STDIO servers run locally on the same machine as devil Code, which has several important implications:
...
- **Execution**: Starts and stops with devil Code (child process lifecycle)
...
- Start when needed by devil Code
- Not require network configuration
- Need to be installed alongside devil Code or via a package manager
...
## Configuring Transports in devil Code

For detailed information on configuring STDIO and SSE transports in devil Code, including example configurations, see the [Understanding Transport Types](using-in-devil-code#understanding-transport-types) section in the Using MCP in devil Code guide.
