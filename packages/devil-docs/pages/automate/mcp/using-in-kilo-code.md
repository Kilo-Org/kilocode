---
title: "Using MCP in devil Code"
description: "How to use MCP servers in devil Code"
---

# Using MCP in devil Code

Model Context Protocol (MCP) extends devil Code's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with devil Code.

{% youtube url="https://youtu.be/6O9RQoQRX8A" caption="Demonstrating MCP installation in devil Code" /%}
...
MCP server configurations are stored inside the main devil config file. There are two levels:

1. **Global Configuration**: `~/.config/devil/devil.jsonc` — applies to all projects.
2. **Project-level Configuration**: `devil.jsonc` in your project root, or `.devil/devil.jsonc` for a cleaner setup.

**Precedence**: Project-level configuration takes precedence over global configuration.

### Editing MCP Settings

You can edit MCP settings from the devil Code settings UI:
...
MCP servers are configured under the `mcp` key in `devil.jsonc`:
...
Remote servers support OAuth 2.0 authentication. If the server supports it, devil Code will automatically start the OAuth flow when you connect. You can also disable OAuth with `"oauth": false`.
...
| **Project** | `.devilcode/mcp.json` | In your project root. Auto-detected by devil Code.               |
...
The CLI accepts several config filenames. The recommended file is `devil.json`:

| Scope       | Recommended Path                     | Also supported                                                 |
| ----------- | ------------------------------------ | -------------------------------------------------------------- |
| **Global**  | `~/.config/devil/devil.json`           | `devil.jsonc`, `opencode.json`, `opencode.jsonc`, `config.json` |
| **Project** | `./devil.json` or `./.devil/devil.json` | `devil.jsonc`, `opencode.jsonc`, `opencode.json`                |
...
_Example of MCP Server config in devil Code (STDIO Transport)_
...
- **Remote (HTTP/SSE)**: Servers hosted over HTTP/HTTPS. devil Code tries `StreamableHTTP` first, then falls back to `SSE` automatically.
...
You can edit both global and project-level MCP configuration files directly from the devil Code settings.

1. Click the {% codicon name="gear" /%} icon in the top navigation of the devil Code pane to open `Settings`.
2. Click the `Agent Behaviour` tab on the left side
3. Select the `MCP Servers` sub-tab
4. Click the appropriate button:
   - **`Edit Global MCP`**: Opens the global `mcp_settings.json` file.
   - **`Edit Project MCP`**: Opens the project-specific `.devilcode/mcp.json` file. If this file doesn't exist, devil Code will create it for you.

{% image src="/docs/img/using-mcp-in-devil-code/mcp-installed-config.png" alt="Edit Global MCP and Edit Project MCP buttons" width="600" caption="Edit Global MCP and Edit Project MCP buttons" /%}
...
{% image src="/docs/img/using-mcp-in-devil-code/using-mcp-in-devil-code-5.png" alt="Delete confirmation box" width="400" caption="Delete confirmation box" /%}
...
| Command           | Description                     |
| ----------------- | ------------------------------- |
| `devil mcp list`   | List all configured MCP servers |
| `devil mcp add`    | Add an MCP server               |
| `devil mcp auth`   | Authenticate with an MCP server |
| `devil mcp logout` | Log out from an MCP server      |
| `devil mcp debug`  | Debug an MCP server connection  |
...
{% image src="/docs/img/using-mcp-in-devil-code/using-mcp-in-devil-code-6.png" alt="Network Timeout pulldown" width="400" caption="Network Timeout pulldown" /%}
...
**In your config file:** Add the tool name (or a wildcard pattern) to the `permission` key in `devil.jsonc`:
...
{% image src="/docs/img/using-mcp-in-devil-code/using-mcp-in-devil-code-7.png" alt="Always allow checkbox for MCP tools" width="120" caption="Always allow checkbox for MCP tools" /%}

When enabled, devil Code will automatically approve this specific tool without prompting. Note that the global "Use MCP servers" setting takes precedence - if it's disabled, no MCP tools will be auto-approved.
...
devil Code does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **devil Marketplace:** Browse community-contributed MCP server configurations and agent skills in the [devil Marketplace](https://github.com/Devil-Org/devil-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask devil Code:** You can ask devil Code to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend devil Code with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, devil Code will automatically detect available tools and resources. To use them:

1. Type your request in the devil Code chat interface
2. devil Code will identify when an MCP tool can help with your task
...
- **Permission Errors:** Ensure proper API keys and credentials are configured in your `mcp_settings.json` (for global settings) or `.devilcode/mcp.json` (for project settings).
- **Tool Not Available:** Confirm the server is properly implementing the tool and it's not disabled in settings
...
- **Server Not Responding:** Check if the server process is running. Use `devil mcp debug <server-name>` to inspect the connection.
- **Permission Errors:** Ensure API keys and credentials are set in your `devil.jsonc` config or via `{env:VARIABLE_NAME}` references.
