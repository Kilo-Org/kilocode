...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows devil Code to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When devil Code needs additional context from external systems
- When devil Code needs to access domain-specific data from specialized MCP servers
- When devil Code needs to retrieve reference documentation hosted by MCP servers
- When devil Code needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
   ...
   - Returns the processed resource data to devil Code for use in the current task
...
- When helping with API development, devil Code retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, devil Code accesses current data samples from connected MCP servers.
- When working in specialized domains, devil Code retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, devil Code references compliance requirements from documentation resources.
