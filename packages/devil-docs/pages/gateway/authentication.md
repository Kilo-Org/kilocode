...
description: "Learn how to authenticate with the devil.ai Gateway using API keys, session tokens, and Bring Your Own Key (BYOK)."
---

# Authentication

The devil.ai Gateway supports multiple authentication methods depending on your use case.
...
API keys are JWT tokens tied to your devil account. See [how to get your API key](/docs/getting-started/setup-authentication#devil-gateway-api-key) for step-by-step instructions.
...
```
X-devilcode-OrganizationId: your_org_id
```
...
BYOK lets you use your own provider API keys with the devil.ai Gateway. When a BYOK key is configured, the gateway routes requests through Vercel AI Gateway using your key. You are billed directly by the provider -- devil does not add any markup.
...
1. Add your provider API key in the devil dashboard or through your devil Code extension settings
...
4. Usage is tracked but not billed to your devil balance (cost is set to $0)
5. If your BYOK key fails, the request will not automatically fall back to devil's keys
...
```
Client → devil Gateway → Vercel AI Gateway (with your key) → Provider
```
...
| `X-devilcode-OrganizationId` | No                      | Organization context for org-scoped requests |
| `X-devilcode-TaskId`         | No                      | Task identifier for prompt cache keying      |
| `X-devilcode-Version`        | No                      | Client version string                        |
| `x-devilcode-mode`           | No                      | Mode hint for `devil-auto` model routing      |
