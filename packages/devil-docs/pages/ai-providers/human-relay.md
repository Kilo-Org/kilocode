...
The Human Relay provider allows you to use devil Code with web-based AI models like ChatGPT or Claude without needing an API key. Instead, it relies on you to manually relay messages between devil Code and the AI's web interface.

## How it Works

1.  **Select Human Relay**: Choose "Human Relay" as your API provider in devil Code's settings. No API key is required.
2.  **Initiate a Request**: Start a chat or task with devil Code as usual.
...
6.  **Paste Back to devil Code**: Return to the dialog box in VS Code, paste the AI's response into the designated field, and click "Confirm".
7.  **Continue**: devil Code will process the response as if it came directly from an API.
