...
In January 2026, Anthropic decided to restrict Claude Code CLI to official Claude Code clients. Claude Code credentials cannot be used in devil Code or other third-party harnesses.

For continued use of Anthropic models in devil Code, please use the [Anthropic API provider](/docs/ai-providers/anthropic) with an API key instead.
{% /callout %}

# Using Claude Code With devil Code

Claude Code is Anthropic's official CLI that provides direct access to Claude models from your terminal. Using Claude Code with devil Code lets you leverage your existing CLI setup without needing separate API keys.
...
If this environment variable is set on your system, the `claude` tool may use it for authentication instead of the interactive `/login` method. When devil Code executes the tool, it will accurately reflect that an API key is being used, as this is the underlying behavior of the `claude` CLI itself.
{% /callout %}
...
Claude Code credentials no longer work in devil Code. Please use the [Anthropic provider](/docs/ai-providers/anthropic) with an API key instead.
{% /callout %}
...
- **Enhance Prompt Feature:** Full compatibility with devil Code's Enhance Prompt feature, allowing you to automatically improve and refine your prompts before sending them to Claude.
