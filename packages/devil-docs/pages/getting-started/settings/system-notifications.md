...
description: "Configure native OS notifications for devil Code"
platform: legacy
---
...
devil Code uses system notifications to inform you about:
...
devil Code's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. devil Code uses macOS's built-in command to display notifications.
...
**How it works:** devil Code first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
