...
description: "Use your own API keys with devil Gateway while retaining platform features"
---

# Bring Your Own Key (BYOK)

Bring Your Own Key (BYOK) lets you use your own API keys when using the devil Gateway, while retaining devil platform features like Code Reviews and Cloud Agents.

A user or organization may want to use BYOK to:

- Utilize new models quickly, devil Gateway supports most new models in minutes
...
devil Gateway currently supports BYOK keys for these providers:
...
1. Log into the devil platform and select the account or organization you want to add the BYOK key to.
2. Navigate to the [Bring Your Own Key (BYOK) page](https://app.devil.ai/byok), available in the sidebar under `Account`.
...
- When you use the **devil Gateway** provider, devil checks if there's a BYOK key for the selected model's provider.
- If a matching BYOK key exists, the request is routed using your key.
- If the key is invalid, the request fails. It does not fall back to using devil's keys.

## Using BYOK in the Extensions and CLI

- BYOK works with the devil Gateway provider. Users should ensure that is set as the active [provider](/docs/ai-providers).
