...
description: "Guide for migrating to devil Code from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to devil Code. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

devil Code supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why devil Code's Rules System?

devil Code simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in devil Code

| Cursor                                      | devil Code                                 | Key Difference                              |
| ------------------------------------------- | ----------------------------------------- | ------------------------------------------- |
| `.cursor/rules/*.mdc` with YAML frontmatter | `.devilcode/rules/*.md` plain Markdown     | No YAML metadata required                   |
...
| Global rules in UI settings                 | `~/.devilcode/rules/*.md` files            | Global rules stored as files in home folder |
...
**2. Create devil Code directory:**

```bash
mkdir -p .devilcode/rules
```
...
for file in .cursor/rules/*.mdc; do
  basename="${file##*/}"
  cp "$file" ".devilcode/rules/${basename%.mdc}.md"
done
...
- Save to `~/.devilcode/rules/cursor-global.md`

**6. Handle legacy `.cursorrules`:**

```bash
cp .cursorrules .devilcode/rules/legacy-rules.md
```

### Converting Cursor's `globs` Patterns

Cursor's `globs` field specifies which files a rule applies to. devil Code handles this through **mode-specific directories** instead.
...
**devil Code approach (Option 1 - Mode-specific directory):**

```bash
mkdir -p .devilcode/rules-code
# Save TypeScript-specific rules here
```

**devil Code approach (Option 2 - Custom mode):**

```yaml
# .devilcodemodes (at project root)
- slug: typescript
  name: TypeScript
...
Then place rules in `.devilcode/rules-typescript/`

### Flattening Nested Cursor Rules

Cursor supports nested `.cursor/rules/` directories. devil Code uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# devil Code: .devilcode/rules/backend-server-api-rules.md
```
...
| Windsurf                                                       | devil Code                      | Key Difference                              |
| -------------------------------------------------------------- | ------------------------------ | ------------------------------------------- |
| `.windsurf/rules/*.md`                                         | `.devilcode/rules/*.md`         | Same Markdown format                        |
| GUI configuration for activation modes                         | File location determines scope | Scope controlled by directory structure     |
| "Always On" mode (GUI)                                         | Place in `.devilcode/rules/`    | Rules stored as files, not GUI settings     |
...
| Global rules in `~/.codeium/windsurf/memories/global_rules.md` | `~/.devilcode/rules/*.md`       | Global rules in home folder, multiple files |
...
```bash
cp .windsurf/rules/*.md .devilcode/rules/
```

**4. Migrate global rules:**

```bash
cp ~/.codeium/windsurf/memories/global_rules.md ~/.devilcode/rules/global-rules.md
```

**5. Handle legacy `.windsurfrules`:**

```bash
cp .windsurfrules .devilcode/rules/legacy-rules.md
```
...
# .devilcode/rules/api-conventions.md
# .devilcode/rules/testing-standards.md
# .devilcode/rules/code-style.md
```

### Converting Windsurf's Activation Modes

Windsurf configures activation through the GUI. In devil Code, file organization replaces GUI configuration:

| Windsurf GUI Mode        | devil Code Equivalent                                        |
| ------------------------ | ----------------------------------------------------------- |
| **Always On**            | Place in `.devilcode/rules/` (default)                       |
...
Then place the rule in `.devilcode/rules-test/`

## AGENTS.md Support

All three tools support the `AGENTS.md` standard. If you have one, it works in devil Code automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - devil Code loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). devil Code also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in devil Code and require user approval to modify.

## Understanding Mode-Specific Rules

This is devil Code's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.

### Directory Structure

```bash
.devilcode/rules/              # Apply to ALL modes
.devilcode/rules-code/         # Only in Code mode
.devilcode/rules-debug/        # Only in Debug mode
.devilcode/rules-ask/          # Only in Ask mode
.devilcode/rules-{custom}/     # Only in your custom mode
```
...
mkdir -p .devilcode/rules-test

# 2. Save rule as plain Markdown
cat > .devilcode/rules-test/testing-standards.md << 'EOF'
# Testing Rules
- Write tests for all features
- Maintain >80% coverage
EOF
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in devil Code panel
- [ ] **Test rule application:** Ask devil Code to perform tasks following your rules
- [ ] **Organize rules:** Split large files, use clear names
- [ ] **Set up mode-specific rules:** Create directories for specialized workflows
- [ ] **Update team docs:** Document new `.devilcode/rules/` location
- [ ] **Commit to version control:** `git add .devilcode/`
...
```bash
ls -la .devilcode/rules/      # Project rules
ls -la ~/.devilcode/rules/    # Global rules
```
...
- **For always-on rules:** Place in `.devilcode/rules/`
...
- **After migrating:** Organize files accordingly in devil Code

### Nested Rules Flattened

Cursor's nested directories don't map to devil Code. Flatten with descriptive names:

```bash
# Bad: .cursor/rules/backend/api/rules.mdc
# Good: .devilcode/rules/backend-api-rules.md
```
...
- **Check setting:** Verify "Use Agent Rules" is enabled in devil Code settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

devil Code supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
```bash
mkdir -p .devilcode/rules-review
mkdir -p .devilcode/rules-docs
```

**Note:** `.devilcodemodes` can be in YAML (preferred) or JSON format. For global modes, edit the `custom_modes.yaml` file via Settings > Edit Global Modes.
...
- [Join our Discord](https://devil.ai/discord) for migration support
