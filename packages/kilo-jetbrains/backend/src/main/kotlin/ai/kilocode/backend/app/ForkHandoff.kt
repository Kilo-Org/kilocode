...
object ForkHandoff {
    ...
    fun forkText(dir: String?): String {
        val lines = mutableListOf(
            "<system-reminder>",
            "This session was forked from an existing session in the current repository or worktree.",
        )
        if (!dir.isNullOrBlank()) {
            lines += "Use this as the current working directory: $dir"
            lines += "For this fork, this location supersedes any earlier repository or worktree location retained in the copied context."
        }
        lines += "The prior conversation context was retained intentionally."
        lines += "The user may continue the same task, explore an alternative approach, or provide new instructions."
        lines += "Follow the user's next instruction as the direction for this fork, using retained context when relevant."
