...
- Only show the worktree row popup for pull requests, not for plain branches without one.
- Show a changes badge on worktree rows that have uncommitted work.
- Rescale Agent Manager list rows correctly when the IDE zoom level changes.
