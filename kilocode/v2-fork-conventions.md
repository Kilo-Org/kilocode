...
`plans/kilo-opencode-v2-issue-13750.md` (scope and phases) and
`kilocode/baseline/pinned-v2-baseline.md` (refs, commands, known-red checks).
...
- To adopt a newer upstream snapshot: fetch, record the new SHA and its merge
  base in `kilocode/baseline/pinned-v2-baseline.md`, merge it into the
...
| Path | Purpose |
|---|---|
| `kilocode/` | Fork documentation and tooling that is not part of any package: this file, `baseline/`, `script/`. |
...
- Do not mark Kilo-owned files. A file under `kilocode/` or `src/kilocode/` is
  Kilo by construction; markers there are noise.
- Preserve existing markers when resolving an upstream merge. Resolve in favour
  ...
  its owned seam.
- Removing a marker means the Kilo behavior is gone. Say so explicitly in the
  slice's report.

## Validation

The authoritative commands come from this v2 tree, not from Kilo main, and are
listed in `kilocode/baseline/pinned-v2-baseline.md`. In short: `bun install`,
...
- Pristine upstream v2 is already red at the pinned SHA. Before attributing a
  ...
  `kilocode/baseline/pinned-v2-baseline.md`, or re-run
  `bun kilocode/script/v2-baseline.ts --checks` on a clean checkout of the pin.
