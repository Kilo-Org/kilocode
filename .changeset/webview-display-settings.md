...
- **Reasoning Display**: choose how reasoning blocks appear while the agent is thinking and after it finishes — collapsed, preview while thinking then collapse, preview while thinking and stay open, full while thinking then collapse, or full and stay open (default). Supersedes the deprecated `auto_collapse_reasoning` boolean, which is still honored for existing configs and maps to the preview-and-stay-open mode it actually produced, so upgrading does not change how reasoning blocks behave.
